...
class PaginaLoginFuncionario extends ConsumerStatefulWidget {
  const PaginaLoginFuncionario({super.key});

  @override
  ConsumerState<PaginaLoginFuncionario> createState() =>
      _PaginaLoginFuncionarioState();
}

class _PaginaLoginFuncionarioState
    extends ConsumerState<PaginaLoginFuncionario> {
  ...
  Widget build(BuildContext context) {
    if (isWebPlatform) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (!mounted) return;
        context.go('/login-empresa?employee-web-denied=1');
      });
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text('Login do Funcionario'),
      ),
      body: AppGradientBackground(
        child: ListView(
          ...
          children: [
            const HeroBanner(
              tag: 'ACESSO DA EQUIPE',
              title: 'Entre para registrar ponto e acompanhar o trabalho.',
              subtitle:
                  'Acesso direto para rotina operacional da equipe, com linguagem mais limpa e objetiva.',
            ),
                ...
                child: GlassCard(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        'Use seu login para registrar ponto, consultar tarefas e acompanhar operacoes do trabalho.',
  ...
  Future<void> _entrar() async {
    ...
    try {
      ...
      if (!rolesPermitidos.contains(role)) {
        ...
        _msg('Este acesso e apenas para funcionario/gerente.');
        return;
      }
