...
- na **Fiscal (prontidao / emissao)**, o que **depende da integracao com a Focus** (configurar integracao global, matriz fiscal editavel, preparar CNPJ, reprocessar automacao, homologacao assistida, chip «Integracao real (Focus)») fica a cargo da **empresa suprema** (`hasSupremePlatformAccess` / tenant da plataforma); o que a **empresa cliente** deve completar, em geral, e: **documentacao para o provisionamento** (p.ex. **certificado**), **dados de cadastro** na tela de **Empresa/Empresas**, **Sincronizar** com a Focus e **emissao**; nao dizer a empresas comuns que «configuram a integracao global» se a UI deles tiver a camada de integracao **travada** — explicar o proximo passo (cadastro, certificado, sync, emissao) sem pedir **token** da Focus; o **token** global e da plataforma
