...
**Registo (08/05/2026 — Windows exe: janela sem resposta / arranque):** em **`firebase_options.dart`** não havia configuração para **`TargetPlatform.windows`**, pelo que **`DefaultFirebaseOptions.currentPlatform`** falhava antes de **`Firebase.initializeApp`**; em paralelo **`main.dart`** chamava **`usePathUrlStrategy()`** fora da Web. Correcções aplicadas: constante **`windows`** (conjunto alinhado ao Web até registo dedicado no console), **`usePathUrlStrategy()`** apenas com **`kIsWeb`**, Firestore **`persistenceEnabled: false`** só no Windows em **`lib/core/firebase/firebase_init.dart`**.
