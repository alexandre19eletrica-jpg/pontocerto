...
- Landing `/vendas-empresa`: foco apenas na empresa — sem atalho de cadastro de escritorio; copy deixa explicito MEI/quem faz cadastro fiscal sem contador nao esta obrigado a contratar pela plataforma; WhatsApp ja existente nesta landing para duvidas/contratar depois se quiserem. Escritorio continua acessivel por `/vendas-contador`, `/cadastro-escritorio-contabil` e `/vendas`.
...
- **Governanca** (`/platform-admin/governanca`): listagem de empresas com `lightweightProfilePending` e escritorios com o mesmo marcador ate concluirem perfil ou vinculos; botoes **Apagar teste** nas listas geridos por callables `platformDeleteLightweightTestCompany` e `platformDeleteLightweightTestOffice` (somente modo leve e sem vinculos ativos / sem carteira; remove Auth utilizador principal e registos minimos).
- **Demo publico** (`publicOpenDemoAccess`): igual dedupe Firestore (`dv1_...`). Callable **sem** Secret Firebase obrigatorio; depende de IAM Token Creator/User da SA runtime sobre `firebase-adminsdk`; na **Governanca**, o livro de demo **`platformListPublicDemoAccessLedger`** mostra apenas agregacao (contagens e datas por perfil), sem exibir rede ou aparelho.
