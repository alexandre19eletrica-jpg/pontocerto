...
- **Demo publico** (`publicOpenDemoAccess`): registo em `platform_public/demo_access` usa **ID de documento deduplicado** por IP (hash) + assinatura leve de navegador/dispositivo (`dv1_...`), mantendo `clientVisitorId` do marketing para correlacao; contagens unicas deixam de multiplicar pela troca de `visitorId` isolada.
