...
- desde **05/05/2026**, um **icone flutuante WhatsApp** (`GlobalWhatsappSupportFab` no `builder` do `MaterialApp`) cobre **todas** as rotas (painel empresa/contador, vendas, pre-cadastro, formularios publicos): abre conversa com suporte usando o mesmo fluxo **`abrirWhatsappVendas`** (numero oficial em `kWhatsappSupportNumberE164`; mensagem curta pré-preenchida para duvidas/informacao); o **stack** do `builder` deve usar **`StackFit.expand`** para o router preencher a viewport em web/desktop
- **Pre-cadastro leve** (`publicCreateCompanyWorkspaceAccess`, `publicCreateAccountantWorkspaceAccess`): o cliente usa **`mapFromCallableData`** ao ler o retorno da Callable; analytics/Meta **nao** devem provocar mensagem de erro unica apos o servidor ter concluido; a notificação interna ao admin após o provisionamento nao pode fazer falhar a Callable (try/log no Functions).
