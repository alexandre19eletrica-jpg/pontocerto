...
- **Demo publico** (`publicOpenDemoAccess`): registo em `platform_public/demo_access` usa **ID de documento deduplicado** por IP (hash) + assinatura leve de navegador/dispositivo (`dv1_...`), mantendo dados no Firestore para nao multiplicar unicas; na **Governanca**, o livro de demo **`platformListPublicDemoAccessLedger`** mostra apenas agregacao (contagens e datas por perfil), sem exibir rede ou aparelho.
