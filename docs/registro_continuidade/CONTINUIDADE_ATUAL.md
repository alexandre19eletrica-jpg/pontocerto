...
- **Documentacao ERP vs Studio (09/05/2026):** os OFICIAL neste repo voltaram a **explicitar** o **Studio Ponto Certo** como produto separado (`studiopontocerto` vs `pontocerto-e1dab`, dois `.exe`). Isso corrige a impressão de «voltou tudo só Ponto Certo» criada quando um commit (**649eaa1**) removeu referências ao cliente IDE externo. Continuidade e comando técnico do Studio: pasta **`C:\StudioPontoCerto`** e **`README_FIREBASE_SETUP.md`** la dentro.

- **Agente de Engenharia (07/05/2026):** UI na **Plataforma**, primeiro submenu **`Agente Engenharia`** — rota **`/platform-admin/agente-engenharia`**; removido cartao na Governanca; redirect **`governanca?v=engineering_agent`** → nova rota. Chat em layout alto (toolbar compacta + area flexivel + input multilinha). Deploy operator confirmado: **`flutter build web --release --no-tree-shake-icons`** + **`firebase deploy --only "functions,hosting"`** (pacote completo quando aplicavel). **Prompt e memoria de conversa no backend** mantem-se como em Functions: system prompt **Ponto Certo** vs **externo**, marcadores `<<<PLANO>>>`…`<<<RESPOSTA>>>`, thread ate **16** mensagens por chamada, modelo/temperatura via mesma config do assistente (`temperature` **0.2**); **Registrar continuidade** grava em `engineering_agent_tasks` + auditoria (nao injeta texto automaticamente no proximo turno).
- **Agente de Engenharia (06/05/2026 — continuacao):** modos **Ponto Certo** (builtin `__pontocerto_builtin__`) vs **projeto externo/novo** com registos `engineering_agent_projects`, sessoes filtradas por `projectId`, `engineering_agent_patches` (draft), `engineeringAgentGenerateCommand`, continuidade gravada via callable; isolamento de prompt/documentacao entre modos; worker em `docs/engineering_agent_worker/README.md`.
...
- **REGRA SUPREMA OPERACIONAL (02/05/2026)**: o Codex fica responsavel **apenas pela edicao de codigo, regras e documentacao tecnica** dentro do repositorio. **Publicacao, build, analyze, deploy, AAB, copia do artefacto e limpeza de cache** passam a ser executados **por voce no seu terminal**. Ao final de **toda** rodada de edicao, o Codex deve entregar os comandos prontos, revisados e no formato correto para PowerShell, evitando erro de execucao no terminal do operador, cobrindo sempre:
  ...
  - `deploy`
  - `AAB`
  - `copiar artefacto para a tela/Desktop`
- **REGRA SUPREMA OPERACIONAL - complemento 02/05/2026**: o Codex **nao deve executar** tarefas pesadas que consumam tempo operacional do usuario, como `analyze`, `build`, `deploy`, `AAB`, limpeza ampla de cache ou outras rotinas demoradas. Essas tarefas ficam com o operador no terminal dele. Ao final de cada rodada, os comandos devem ser entregues **em uma unica sequencia**, prontos para **copiar e colar de uma vez no PowerShell**, sem blocos separados e sem depender de ajuste manual.
- **REGRA SUPREMA OPERACIONAL - blindagem de execucao 02/05/2026**: neste ambiente, os comandos finais devem evitar `pub get` automatico durante `analyze` e `build appbundle`, usando `--no-pub`, porque o `pub.dev` pode devolver erro externo de advisories (`advisoriesUpdated must be a String`) sem relacao com o codigo alterado. A sequencia final tambem deve:
  - **parar no primeiro erro real**
  - **nao seguir para deploy se analyze/build falharem**
  - **nao copiar AAB se o ficheiro nao existir**
  - evitar publicar `hosting` com `build/web` antigo apos falha de compilacao
  - se tiver havido `flutter clean` na execucao anterior, a proxima sequencia deve **comecar com `flutter pub get`** para recriar `.dart_tool/package_config.json` antes de usar `--no-pub`
  - comandos com `Copy-Item`, `Move-Item`, `Remove-Item` e chamadas com parametros longos devem ser entregues **em uma unica linha**, sem quebra entre o comando e os parametros, para o PowerShell nao interpretar `-Destination` ou flags semelhantes como um novo comando
  - quando a rodada tiver alteracao em **Flutter web** (tela, rota, copy, fluxo, navegação, widgets, landing, shell ou qualquer ficheiro em `lib/` que afete a interface), a publicacao precisa incluir **`hosting`**; subir apenas `functions` nao publica essas alteracoes visuais
- **Registo 02/05/2026 - Demo publico com identidade ficticia**: o demo publico deixou de depender de empresa real para abrir. O backend agora garante um workspace ficticio proprio (`public_demo_workspace`) com dados de apresentacao da **empresa `Ponto Certo`** e do **contador `Escritorio Ponto Certo`**, preservando o menu real em sessao `demoReadOnly` e evitando exposicao de dados reais nas paginas publicas e nos acessos de venda.
- **Registo 03/05/2026 - Correcao de entrada publica e demo**: as rotas publicas de pre-cadastro passaram a permanecer acessiveis mesmo quando existir sessao ativa, evitando bloqueio ao sair do demo para o acesso real. O demo recebeu reforco no login por `customToken`, com garantia de usuario auth demo no backend e `signOut` controlado antes do novo login no cliente. Nas landings segmentadas, cada pagina passou a exibir apenas o demo correspondente (`empresa` na pagina da empresa, `contador` na pagina do contador) com copy comercial de demonstracao mais forte.
- **Registo 03/05/2026 - CTA comercial e acesso demo estabilizados**: o fluxo comercial publico foi reconduzido para a regra vigente: `inicio`, `sales_page`, `vendas_empresa` e o alias `/contratar` agora levam para `/cadastro-escritorio-contabil`, evitando que o lead caia no formulario errado. No backend, `publicOpenDemoAccess` passou a tolerar reutilizacao de usuario demo por e-mail no Firebase Auth, reduzindo o `internal` que ainda aparecia em `empresa demo` e `contador demo`.
- **Registo 03/05/2026 - Emails de acesso alinhados ao comercial atual**: acessos de escritorio e empresa agora disparam e-mail com link para criar senha, link web de acesso e aviso claro de que o app da Play Store ainda esta em teste e depende de liberacao do acesso antes do uso. Os cadastros leves (`publicCreateAccountantWorkspaceAccess` e `publicCreateCompanyWorkspaceAccess`) tambem passaram a disparar esse envio quando a configuracao de e-mail estiver disponivel.
- **Registo 03/05/2026 - Pre-cadastro tambem notifica a plataforma**: o fluxo comercial de `pre_registered` passou a disparar notificacao interna administrativa, para que voce receba o alerta mesmo quando o lead ainda esta na etapa anterior ao cadastro efetivo. O cadastro direto completo da empresa tambem passou a enviar o e-mail de acesso no mesmo padrao de senha + link web + aviso da Play Store em teste.
- **Registo 03/05/2026 - Pre-cadastro agora ja envia criacao de senha**: o `pre_registered` comercial passou a criar acesso leve tanto para o escritorio indicado quanto para a empresa informada, ja enviando e-mail com criacao de senha e link web de acesso nessa primeira etapa. O cadastro real posterior da empresa foi ajustado para reaproveitar esse acesso leve sem travar por e-mail duplicado, marcando o workspace provisiorio anterior como superseded quando houver migracao para o cadastro definitivo.
- **Registo 03/05/2026 - Regra confirmada de publicacao**: ficou comprovado em terminal que `cmd /c npm.cmd --prefix functions run build` + `firebase deploy --only functions --project pontocerto-e1dab` publicam **backend**, e que `flutter build appbundle --release --no-pub` + `Copy-Item ... pontocerto-ULTIMA.aab` geram/copiam o **artefacto Android**. Isso **nao publica a interface web**. Sempre que a rodada incluir alteracoes visuais ou de fluxo em Flutter web, e obrigatorio executar tambem `build web` + `firebase deploy --only hosting` (ou `hosting,functions`). Se publicar so `functions`, a percepcao correta pode ser "nao subiu as atualizacoes" porque a UI continua antiga.
- **Registo 03/05/2026 - Landing empresa no modelo atual**: a pagina `vendas_empresa` deixou de trabalhar no fluxo "indique seu contador". O CTA principal agora leva a empresa direto para `/cadastro-empresa`, e a copy foi reposicionada para o modelo atual: empresa entra primeiro, organiza a operacao, e o contador entra depois no fluxo sem travar o inicio do uso.
- **Registo 03/05/2026 - Fiscal / competencia automatica**: quando a competencia atual vier vazia, mas existirem notas em competencias anteriores, a tela fiscal passa a reposicionar automaticamente para a competencia mais recente com documentos, reduzindo o efeito de "nota sumida" por mudanca de mes.
- **Registo 02/05/2026 - Demo publico em leitura + protecao fiscal**: foi aberta a base do **acesso demo publico** para `empresa` e `contador`, usando o **menu real do sistema** com sessao marcada como `demoReadOnly`. Entraram rotas publicas de demo, autenticacao por `customToken`, banner de somente leitura, CTA nas paginas de vendas e contagem agregada de acessos demo no painel da plataforma. Tambem foram aplicadas travas de escrita em Functions e Firestore para pagamentos, dividas, configuracoes da empresa, usuarios/funcionarios e `service_invoices`, evitando alteracao de dados pela sessao demo. No fiscal, a leitura por competencia passou a usar **data de referencia com fallback** (`officialIssuedAt` -> `issueDate` -> `serviceDate` -> `createdAt`) tambem em relatorios/PDF, reduzindo o risco de "nota sumida" quando a emissao real nao preenche `issueDate` como esperado.
- **Registo 02/05/2026 - XML do contador via Focus**: a rota `Declaracoes e obrigacoes` do contador ganhou captura de XML fiscal recebida via Focus usando sempre a **empresa ativa** da carteira. A sincronizacao usa o CNPJ da empresa atual, grava em `fiscal_received_documents` por `companyId` e hoje cobre **NF-e recebidas** e **NFS-e nacionais recebidas**. O download aparece quando a Focus entrega o XML completo no payload; manifestacao do destinatario e tratamento de XML grande em Storage ficam para rodada posterior.
- **Registo 02/05/2026** (ver `REGISTRO_ATUALIZACOES` **2026-05-02**): build Android gerada com sucesso em **`1.0.87+1058`** (`pontocerto-1.0.87-1058.aab`), com consolidacao final do fechamento trabalhista por competencia e correcao de build nas telas de governanca, folha e apoio operacional do modulo `Trabalhista`. O estado verificavel foi regenerado e os documentos oficiais passaram a apontar esta versao como referencia atual.
