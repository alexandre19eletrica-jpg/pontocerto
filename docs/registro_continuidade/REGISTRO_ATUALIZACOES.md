...
- **Correccao (Web) — WhatsApp nas landings empresa/contador**: o primeiro `await` (`canLaunchUrl` / ciclo antes de `launchUrl`) fazia o navegador perder o gesto do clique e **bloqueava** a nova aba para `wa.me`; passou-se a `window.open` sincrono na variante browser (`vendas_whatsapp_web_open_web.dart`) + stub; nativo continua com `launchUrl`; eventos fbq/analytics ficam **depois** da abertura (`vendas_whatsapp_button.dart`). OFICIAL 01–04 actualizados.
