...
- **Correccao (Web) — WhatsApp nas landings empresa/contador (evoluçao apos Pixel)**: alem de evitar `await` antes da abertura, o **Flutter Web `Link`** (`package:url_launcher`) substitui `window.open` manual: heroi (`secondaryWhatsappMessage`), rodape (`whatsappFooterPrefill`) e botao WhatsApp grande usam **`followLink()`** sincrono com o DOM; **`scheduleWhatsappComercialSignals`** corre **depois** para o Meta / `sales_whatsapp_comercial` não bloquearem a navegacao. Removidos `vendas_whatsapp_web_open_*.dart`. Ficheiros: `vendas_whatsapp_button.dart`, `vendas_landing_ui.dart`, `vendas_empresa_page.dart`, `vendas_contador_page.dart`. OFICIAL 01–04 actualizados.
