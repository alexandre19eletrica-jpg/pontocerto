...
- Plataforma `/platform-admin/governanca` (submenu **Governanca** sob Plataforma): duas listas em cards expansiveis — empresas em **teste direto** (cadastro leve sem escritorio completo) e **livro de demos** (registos anonimizados: hash de IP parcial, tipo de dispositivo, telas); lista de **escritorios em teste leve** com contagem de carteira; botoes **Apagar** com confirmacao dialogo (somente cadastros leveis e sem vinculo ativo empresa-contador / sem empresas na carteira do escritorio); botao para ir a **Escritorios e carteira**; atalho **Financeiro** por linha de empresa teste.
