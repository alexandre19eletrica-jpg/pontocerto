...
const vendas_public_flow_1 = require("./vendas_public_flow");
...
function roleParaFirestore(valor) {
    ...
    if (normalizado === 'owner' || normalizado === 'dono')
        return 'OWNER';
    if (normalizado === 'manager' || normalizado === 'gerente')
        return 'MANAGER';
    if (normalizado === 'accountant' || normalizado === 'contador')
        return 'ACCOUNTANT';
    return 'EMPLOYEE';
}
function isVisibleEmployeeRole(valor) {
    const normalizado = normalizarRole(valor);
    return (normalizado === 'employee' ||
        normalizado === 'manager' ||
        normalizado === 'accountant' ||
        normalizado === 'unknown');
}
const MAIL_FROM = (0, params_1.defineString)('MAIL_FROM');
const SMTP_USER = (0, params_1.defineString)('SMTP_USER');
const SMTP_APP_PASSWORD = (0, params_1.defineString)('SMTP_APP_PASSWORD');
const SMTP_HOST = (0, params_1.defineString)('SMTP_HOST');
const SMTP_PORT = (0, params_1.defineString)('SMTP_PORT');
const SMTP_SECURE = (0, params_1.defineString)('SMTP_SECURE');
const SENDGRID_KEY = (0, params_1.defineString)('SENDGRID_KEY');
const APP_APK_URL = (0, params_1.defineString)('APP_APK_URL');
const APPDISTRIBUTION_APP_ID = (0, params_1.defineString)('APPDISTRIBUTION_APP_ID');
...
function obterConfigEmail() {
    ...
    return {
        ...
        smtpSecure: ['1', 'true', 'yes', 'on'].includes(SMTP_SECURE.value().trim().toLowerCase()),
        fromEmail: MAIL_FROM.value().trim(),
        apkUrl: configuredApkUrl || DEFAULT_PLAY_STORE_URL,
        appDistributionAppId: APPDISTRIBUTION_APP_ID.value().trim() ||
            '1:1074663509241:android:882c17b485b01c7aa06628',
    };
}
function missingInviteConfig(cfg) {
    const missing = [];
    ...
    if (smtpConfigured) {
        ...
        if (!cfg.smtpAppPassword)
            missing.push('smtp.app_password');
    }
    else if (sendgridConfigured) {
        if (!cfg.sendgridKey)
            missing.push('sendgrid.key');
    }
    else {
    ...
    if (!cfg.fromEmail)
        missing.push('mail.from');
    return missing;
}
function obterConfigAsaas() {
    const apiKey = ASAAS_API_KEY.value().trim();
    const environmentRaw = ASAAS_ENVIRONMENT.value().trim().toLowerCase();
    const environment = environmentRaw === 'production' || environmentRaw === 'prod'
        ? 'production'
        : 'sandbox';
...
function obterConfigFocusPlatform() {
    const environment = String(process.env.FOCUS_DEFAULT_ENVIRONMENT ?? '').trim().toLowerCase() ||
        'homologacao';
    return {
...
function assertAsaasConfigured() {
    ...
    if (!cfg.apiKey) {
        throw new functions.https.HttpsError('failed-precondition', 'ASAAS_API_KEY nao configurada nas Functions.');
    }
    return cfg;
}
function obterConfigAssistant() {
    return {
...
async function addTesterToFirebaseAppDistribution(params) {
    ...
    if (appIdParts.length < 2) {
        throw new functions.https.HttpsError('failed-precondition', 'appdistribution.app_id invalido.');
    }
    ...
    const url = `https://firebaseappdistribution.googleapis.com/v1/projects/${projectNumber}/apps/${encodeURIComponent(appId)}/testers:batchAdd`;
    ...
    if (res.status < 200 || res.status >= 300) {
        throw new functions.https.HttpsError('internal', 'Falha ao adicionar tester no App Distribution.');
    }
}
...
function assertClaims(context) {
    ...
    if (!companyId || !employeeId || !token.role) {
        throw new functions.https.HttpsError('failed-precondition', 'Custom claims ausentes. Execute authSyncClaims antes de usar esta operacao.');
    }
    return { uid, companyId, role, employeeId };
}
function assertRole(claims, allowed) {
...
function assertSupremePlatformAccess(claims) {
    if (claims.role !== 'OWNER' || !isSupremePlatformCompany(claims.companyId)) {
        throw new functions.https.HttpsError('permission-denied', 'Apenas a empresa suprema da plataforma pode usar esta operacao.');
    }
}
function assertObservabilityAccess(claims) {
    assertRole(claims, ['OWNER', 'MANAGER', 'ACCOUNTANT']);
}
function assertCompany(companyId, claims) {
    if (companyId !== claims.companyId) {
        throw new functions.https.HttpsError('permission-denied', 'Acesso cross-company bloqueado.');
    }
}
function parsePositiveInt(valor, nomeCampo) {
...
function parseNonNegativeInt(valor, nomeCampo) {
    const n = Number(valor);
    if (!Number.isInteger(n) || n < 0) {
        throw new functions.https.HttpsError('invalid-argument', `${nomeCampo} invalido.`);
    }
...
function parseOptionalDate(valor, nomeCampo) {
...
function normalizePaymentType(explicitValue, fallbackCompensationType) {
...
function paymentLaunchReferenceLabel(data) {
    const paidAt = data.paidAt;
    const createdAt = data.createdAt;
    const dueDate = data.dueDate;
    const timestampValue = paidAt instanceof admin.firestore.Timestamp
        ? paidAt
...
async function carregarUsuarioMesmoTenant(uid, claims) {
...
async function carregarOwnerDaEmpresa(companyId) {
...
async function assertOperatorFromProfile(context) {
    const { uid, token } = assertAuth(context);
    ...
    const claims = { uid, companyId, role, employeeId };
    assertRole(claims, ['OWNER', 'MANAGER', 'ACCOUNTANT']);
    return claims;
}
...
function asFocusDecimal(value, fallback = 0) {
...
function asUpperStatus(value, fallback = 'EMITTED') {
    const normalized = asTrimmedString(value).toUpperCase();
    return normalized || fallback;
}
function accountantPermissions(settingsData) {
    return asRecord(settingsData.accountantPermissions);
}
function accountantCanManageFiscalInvoices(settingsData) {
...
function assertCanOperateFiscalInvoices(params) {
    if (params.claims.role === 'ACCOUNTANT' && !accountantCanManageFiscalInvoices(params.settingsData)) {
        throw new functions.https.HttpsError('permission-denied', 'O contador desta empresa esta em modo somente leitura para emissao fiscal.');
    }
}
function assistantSettings(settingsData) {
    return asRecord(settingsData.assistantSettings);
}
function assistantUsage(settingsData) {
    return asRecord(settingsData.assistantUsage);
}
function currentAssistantPeriodKey(date = new Date()) {
    const year = date.getUTCFullYear();
    const month = String(date.getUTCMonth() + 1).padStart(2, '0');
    return `${year}-${month}`;
}
function assistantRequestLimit(settingsData) {
...
function assistantEnabledForRole(settingsData, role) {
    const settings = assistantSettings(settingsData);
    if (settings.enabled === false)
        return false;
    if (role === 'EMPLOYEE')
        return settings.allowEmployeeAccess !== false;
    if (role === 'ACCOUNTANT')
        return settings.allowAccountantAccess !== false;
    if (role === 'MANAGER')
        return settings.allowManagerAccess !== false;
    return true;
}
function assertCanUseAssistant(params) {
    if (!assistantEnabledForRole(params.settingsData, params.claims.role)) {
        throw new functions.https.HttpsError('permission-denied', 'O Assistente Inteligente nao esta liberado para este perfil nesta empresa.');
    }
    const periodKey = currentAssistantPeriodKey();
    const usage = assistantUsage(params.settingsData);
    const currentPeriodUsage = asTrimmedString(usage.periodKey) === periodKey ? Number(usage.requestCount ?? 0) : 0;
    const requestLimit = assistantRequestLimit(params.settingsData);
    const blockWhenLimitReached = assistantSettings(params.settingsData).blockWhenLimitReached !== false;
    if (blockWhenLimitReached &&
        requestLimit > 0 &&
        Number.isFinite(currentPeriodUsage) &&
        currentPeriodUsage >= requestLimit) {
        throw new functions.https.HttpsError('resource-exhausted', 'A franquia mensal do assistente foi atingida para esta empresa.');
    }
}
const ASSISTANT_MIN_INTERVAL_MS = 1500;
const ASSISTANT_BURST_WINDOW_MS = 60000;
const ASSISTANT_BURST_MAX_REQUESTS = 20;
async function reserveAssistantBurstQuota(claims) {
    ...
    await admin.firestore().runTransaction(async (transaction) => {
        ...
        if (Number.isFinite(lastRequestAtMs) && lastRequestAtMs > 0) {
            ...
            if (elapsedMs >= 0 && elapsedMs < ASSISTANT_MIN_INTERVAL_MS) {
                throw new functions.https.HttpsError('resource-exhausted', 'Aguarde um instante antes de enviar outra mensagem ao assistente.');
            }
        }
        const windowStillActive = Number.isFinite(currentWindowStartMs) &&
        ...
        if (nextRequestCount > ASSISTANT_BURST_MAX_REQUESTS) {
            throw new functions.https.HttpsError('resource-exhausted', 'Muitas mensagens em pouco tempo. Aguarde um minuto e tente novamente.');
        }
...
function buildDefaultPublicSalesConfig(current) {
    ...
    return {
        ...
        additionalAccess: {
            ...
            title: asTrimmedString(additionalAccess.title) ||
...
function marketingDateKey(value = new Date()) {
    return value.toISOString().slice(0, 10);
}
function marketingReferrerHost(referrer) {
...
function marketingEventScore(eventName) {
    switch (eventName) {
...
function publicSalesPlanEntries(config) {
    return [
...
async function fetchAsaasCustomer(cfg, customerId) {
    if (!customerId.trim())
        return {};
    return asaasRequest(cfg, `/customers/${encodeURIComponent(customerId)}`);
}
function billingIntegrationSettings(currentCommercial) {
    return asRecord(currentCommercial.billingIntegration);
}
function buildDefaultBillingIntegration(currentCommercial) {
...
function billingAccessState(commercial) {
...
function buildDefaultCommercialSettings(current) {
    const inferredCompanyId = asTrimmedString(current.companyId) ||
    ...
    const contractedAppUsers = Number(currentCommercial.contractedAppUsers ?? seatsIncluded) || seatsIncluded;
    const baseSystemPriceCents = Number(currentCommercial.baseSystemPriceCents ?? currentCommercial.monthlyPriceCents ?? 0) || 0;
    const extraAppUserPriceCents = Number(currentCommercial.extraAppUserPriceCents ?? 0) || 0;
    const additionalAppUsers = Math.max(0, contractedAppUsers - seatsIncluded);
    const calculatedMonthlyPriceCents = baseSystemPriceCents + additionalAppUsers * extraAppUserPriceCents;
    ...
    const defaults = {
        ...
        lifecycleStatus: asTrimmedString(currentCommercial.lifecycleStatus) || 'trial',
        ...
        approvalStatus: asTrimmedString(currentCommercial.approvalStatus) || 'auto_approved',
        seatsIncluded,
        contractedAppUsers,
        baseSystemPriceCents,
        extraAppUserPriceCents,
        pricingModel: asTrimmedString(currentCommercial.pricingModel) || 'base_plus_app_users',
        monthlyPriceCents: Number(currentCommercial.monthlyPriceCents ?? calculatedMonthlyPriceCents) ||
            calculatedMonthlyPriceCents,
        calculatedMonthlyPriceCents,
        accessControlMode: asTrimmedString(currentCommercial.accessControlMode) || 'standard',
        activationRequired: currentCommercial.activationRequired === true,
        activationStatus: asTrimmedString(currentCommercial.activationStatus) ||
...
function activationCodeLast4(value) {
...
function webhookTokenFromRequest(req) {
    ...
    if (Array.isArray(direct))
        return asTrimmedString(direct[0]);
    if (direct != null)
        return asTrimmedString(direct);
...
async function asaasRequest(cfg, path, init) {
    ...
    if (!response.ok) {
        ...
        const description = asTrimmedString(firstError.description) ||
...
async function syncAsaasSubscriptionForCommercialSettings(params) {
    ...
    const nextDueDateTs = parseTimestampLike(billing.currentPeriodEnd) ??
        admin.firestore.Timestamp.fromDate(new Date(Date.now() + 24 * 60 * 60 * 1000));
    const description = `Plano ${asTrimmedString(params.commercial.plan)} | ${params.companyId}`;
    const result = await asaasRequest(cfg, `/subscriptions/${encodeURIComponent(subscriptionId)}`, {
        method: 'PUT',
        body: {
            customer: customerId,
            ...
            description,
            externalReference: asTrimmedString(billing.externalReference) || params.companyId,
    ...
    return {
        ...
        currentPeriodEnd: parseTimestampLike(result.nextDueDate) ?? billing.currentPeriodEnd ?? null,
        lastWebhookAt: billing.lastWebhookAt ?? null,
    };
}
function billingChargeSortTime(payment) {
    const value = parseTimestampLike(payment.dueDate) ??
...
function selectAsaasRenewalPayment(payments) {
...
function primaryAsaasPaymentUrl(payment) {
    return (asTrimmedString(payment.invoiceUrl) ||
        asTrimmedString(payment.bankSlipUrl) ||
        asTrimmedString(payment.transactionReceiptUrl));
}
function nextCancellationAccessDeadline(billing) {
    ...
    return admin.firestore.Timestamp.fromMillis(Date.now() + Math.max(0, graceDays) * 24 * 60 * 60 * 1000);
}
function asaasEventPaymentData(payload) {
    return asRecord(payload.payment);
}
function asaasEventSubscriptionData(payload) {
    return asRecord(payload.subscription);
}
async function resolveCompanyIdFromAsaasPayload(payload) {
    const payment = asaasEventPaymentData(payload);
    const subscription = asaasEventSubscriptionData(payload);
    const externalReference = asTrimmedString(payment.externalReference || subscription.externalReference || payload.externalReference);
...
function billingStatusFromAsaasEvent(eventName, paymentData) {
...
function runtimeSummaryRef(companyId) {
    return admin.firestore().collection('company_runtime_summary').doc(companyId);
}
function paymentSummaryBucket(data) {
    const status = asTrimmedString(data.status).toUpperCase();
    if (status === 'PENDING')
        return 'pendingPayrollCents';
    if (status === 'PAID')
        return 'paidPayrollCents';
    if (status === 'CONFIRMED')
        return 'confirmedPayrollCents';
    if (status === 'CONTESTED')
        return 'contestedPayrollCents';
    return null;
}
function paymentSummaryAmount(data) {
    const direct = Number(data.netCents ?? data.valorCents ?? data.grossCents ?? 0);
    if (!Number.isFinite(direct))
        return 0;
    return Math.max(0, Math.trunc(direct));
}
function debtSummaryBucket(data) {
    const status = asTrimmedString(data.status).toUpperCase();
    const type = asTrimmedString(data.type).toUpperCase();
    if (type === 'ADVANCE') {
        if (status === 'OPEN')
            return 'openAdvancesCents';
        if (status === 'SETTLED')
            return 'settledAdvancesCents';
        return null;
    }
    if (status === 'OPEN')
        return 'openDebtsCents';
    if (status === 'SETTLED')
        return 'settledDebtsCents';
    return null;
}
function debtSummaryAmount(data) {
    const direct = Number(data.amountCents ?? data.valorCents ?? 0);
    if (!Number.isFinite(direct))
        return 0;
    return Math.max(0, Math.trunc(direct));
}
async function applyRuntimeSummaryDelta(tx, companyId, section, bucketBefore, amountBefore, bucketAfter, amountAfter, currentSectionData) {
    ...
    if (bucketBefore) {
        ...
        next[bucketBefore] = Math.max(0, Math.trunc((Number.isFinite(currentValue) ? currentValue : 0) - amountBefore));
    }
    if (bucketAfter) {
        const currentValue = Number(next[bucketAfter] ?? 0);
        next[bucketAfter] = Math.max(0, Math.trunc((Number.isFinite(currentValue) ? currentValue : 0) + amountAfter));
    }
...
async function readRuntimeSummarySection(tx, companyId, section) {
    const snap = await tx.get(runtimeSummaryRef(companyId));
    return asRecord(asRecord(snap.data())[section]);
}
function financeMovementSummaryFields(data) {
...
async function applyFinanceMovementSummaryDelta(tx, companyId, beforeData, afterData) {
    const ref = runtimeSummaryRef(companyId);
    const snap = await tx.get(ref);
    const current = asRecord(snap.data());
    const finance = { ...asRecord(current.finance) };
    function addToBucket(stringKey, amount, multiplier) {
        if (!stringKey || amount <= 0)
            return;
        const currentValue = Number(finance[stringKey] ?? 0);
        finance[stringKey] = Math.max(0, Math.trunc((Number.isFinite(currentValue) ? currentValue : 0) + amount * multiplier));
    }
 ...
 */
function extractFocusOfficialNfseNumber(r) {
    const directKeys = [
        'numero_nfse',
        'numeroNfse',
...
function coalesceFocusNfseStatusRawForNormalize(r) {
    return (asTrimmedString(r.status) ||
        asTrimmedString(r.situacao) ||
        asTrimmedString(r.Situacao) ||
        asTrimmedString(r.situacao_nfe) ||
        asTrimmedString(r.situacaoNfe) ||
        asTrimmedString(r.situacao_nfse) ||
        asTrimmedString(r.estado) ||
        (typeof r.codigo_situacao === 'number'
            ? String(r.codigo_situacao)
            : '') ||
        (typeof r.codigoSituacao === 'number'
            ? String(r.codigoSituacao)
            : '') ||
        '');
}
function fiscalInvoiceSummaryFields(data) {
    const status = asTrimmedString(data.status).toUpperCase();
    ...
    const fromOfficialResponse = extractFocusOfficialNfseNumber(asRecord(data.officialResponse));
...
async function rebuildFiscalInvoiceRuntimeSummary(companyId) {
    ...
    for (const doc of snapshot.docs) {
        ...
        if (summary.countBucket === 'draftInvoicesCount')
            draftInvoicesCount += 1;
        if (summary.countBucket === 'approvedInvoicesCount') {
            approvedInvoicesCount += 1;
            emittedGrossAmountCents += summary.grossAmountCents;
        }
        if (summary.countBucket === 'canceledInvoicesCount')
            canceledInvoicesCount += 1;
        if (summary.countBucket === 'processingInvoicesCount')
            processingInvoicesCount += 1;
    }
...
function companyAccessState(settingsData) {
    const inferredCompanyId = asTrimmedString(settingsData.companyId) || asTrimmedString(settingsData.id);
    if (isSupremePlatformCompany(inferredCompanyId)) {
        return {
            allowLogin: true,
            lifecycleStatus: 'released',
...
function assertPlatformAdmin(claims, userProfile) {
    ...
    if (claims.role !== 'OWNER' || (!companyAllowed && !emailAllowed)) {
        throw new functions.https.HttpsError('permission-denied', 'Apenas a administracao da plataforma pode executar esta operacao.');
    }
}
function assistantThreadTitleFromMessage(message) {
...
function buildAssistantFeatureInventory() {
    return [
...
function buildAssistantRouteGuide(route) {
    switch (route) {
        case '/workforce':
            return [
                'Guia da tela atual:',
...
function buildAssistantFaqGuide() {
    return [
...
function buildAssistantModuleResponseGuide() {
    return [
...
function buildAssistantBusinessContextGuide(companySettings, userProfile) {
    const companyData = asRecord(companySettings.companyData || userProfile.companyData);
...
function buildAssistantInstructions(params) {
    const companyName = asTrimmedString(params.userProfile.companyName ??
    ...
    const focusApi = asTrimmedString(fiscalIntegration.focusNfseApi ?? fiscalRouting.focusNfseApi);
    const companyProfile = asTrimmedString(params.companySettings.companyOperationalProfile) || 'small_business';
    ...
    const businessContextGuide = buildAssistantBusinessContextGuide(params.companySettings, params.userProfile);
    return [
...
async function buildSystemIssuesSummary(companyId) {
    const snap = await admin
        .firestore()
...
async function buildRuntimeIncidentsSummary(companyId) {
    const snap = await admin
        .firestore()
...
async function createOpenAiResponse(params) {
    ...
    if (!response.ok) {
        ...
        const message = asTrimmedString(errorData.message) ||
...
function summarizeAssistantPayloadForLogs(payload) {
    const output = Array.isArray(payload.output) ? payload.output : [];
    return {
        id: asTrimmedString(payload.id),
        ...
        outputSummary: output.slice(0, 3).map((item) => {
            ...
            return {
                ...
                contentPreview: content.slice(0, 2).map((part) => {
                    const piece = asRecord(part);
                    return (asTrimmedString(piece.output_text) ||
                        asTrimmedString(piece.text) ||
                        asTrimmedString(asRecord(piece.text).value) ||
                        asTrimmedString(piece.value) ||
                        asTrimmedString(piece.refusal) ||
...
function buildAssistantFallbackReply(payload) {
...
function normalizeOperationNatureCode(value) {
    const text = asTrimmedString(value);
    if (!text)
        return '1';
    if (/^[0-9]+$/.test(text)) {
    ...
    const normalized = text
        ...
        .normalize('NFD')
        .replace(/[\u0300-\u036f]/g, '');
    if (normalized.includes('fora do municipio') ||
 ...
 */
function resolveAuthForCustomTokens() {
    ...
    try {
        ...
        const existingSecondary = admin.apps.some((application) => !!application &&
...
function systemIssueRef(issueId) {
    return admin.firestore().collection('system_issues').doc(issueId);
}
function buildSystemIssueFingerprint(incidentData) {
...
function analyzeRuntimeIncidentHeuristics(incidentData) {
    ...
    if (joined.includes('focus') ||
        ...
        return {
            summary: 'Falha classificada como fiscal/integracao. O caminho mais seguro e reprocessar o provisionamento fiscal da empresa antes de qualquer nova emissao.',
            recommendedAction: 'Reprocessar a automacao fiscal da empresa e revisar token, certificado e rota fiscal recomendada.',
    ...
    if (joined.includes('permission') ||
        ...
        return {
            summary: 'Falha classificada como permissao/acesso. O sistema nao deve tentar corrigir isso sozinho sem revisao humana.',
            recommendedAction: 'Revisar perfil, claims e regras de acesso da empresa antes de repetir a operacao.',
    ...
    if (joined.includes('timeout') ||
        ...
        return {
            summary: 'Falha classificada como conectividade temporaria. A recomendacao e repetir a operacao apos conferir internet e disponibilidade do servico externo.',
            recommendedAction: 'Tentar novamente a operacao apos validar conectividade e disponibilidade do integrador.',
    ...
    return {
        summary: 'Falha classificada como incidente generico de aplicacao. O sistema registrou a ocorrencia, mas ainda nao ha playbook automatico seguro para este caso.',
        recommendedAction: 'Analisar a mensagem, a tela e o stack trace antes de decidir uma acao manual.',
...
function focusProviderErrorMessage(data, fallback) {
    const direct = asTrimmedString(data.mensagem) ||
...
function focusProviderErrorDetailsText(details) {
...
function classifyFocusProviderError(rawMessage, detailsText = '') {
    ...
    const detailSuffix = raw && raw !== detailsText ? ` Detalhe Focus: ${raw}` : '';
    if (code === 'E0310' ||
        ...
        return ('Codigo de tributacao nacional do ISS invalido para a NFS-e Nacional. ' +
            'Revise o servico com codigo nacional no padrao XX.XX.XX e CNAE com 7 digitos.' +
            detailSuffix);
    }
    if (normalized.includes('codigo_tributario_municipio') ||
        normalized.includes('codigo tributario municipio') ||
        normalized.includes('ctribmun')) {
        return ('Codigo tributario municipal invalido para a prefeitura configurada. ' +
            'Revise o codigo municipal do servico conforme o cadastro aceito pela Focus para esse municipio.' +
            detailSuffix);
    }
    if (normalized.includes('item_lista_servico') ||
        normalized.includes('item lista servico') ||
        normalized.includes('lista de servico')) {
        return ('Item da lista de servico invalido. Revise a classificacao do servico conforme LC 116 e a rota fiscal da empresa.' +
            detailSuffix);
    }
    if (normalized.includes('cnae')) {
        return ('CNAE invalido para emissao. Revise o CNAE principal do servico no padrao de 7 digitos.' +
            detailSuffix);
    }
    if (normalized.includes('inscricao municipal') ||
        normalized.includes('inscricao_municipal') ||
        normalized.includes('im do prestador')) {
        return ('Inscricao municipal do prestador invalida ou nao habilitada no provedor. ' +
    ...
    if (normalized.includes('certificado') ||
        ...
        return ('Certificado digital invalido, expirado ou divergente do CNPJ da empresa. ' +
            'Revise o certificado A1 e a senha cadastrada na Focus.' +
            detailSuffix);
    }
    if (normalized.includes('logradouro_tomador') ||
        ...
        return ('Endereco do tomador incompleto para o layout fiscal exigido. ' +
            'Revise logradouro, numero, bairro, CEP e municipio do cliente.' +
            detailSuffix);
    }
    if (normalized.includes('cno') || normalized.includes('obra')) {
        return ('Dados da obra invalidos ou incompletos para esse codigo de servico. ' +
    ...
    if (normalized.includes('duplic') ||
        ...
        return ('Ja existe uma solicitacao fiscal com essa referencia na Focus. ' +
            'Consulte a nota existente antes de tentar reenviar.' +
            detailSuffix);
    }
    if (normalized.includes('processando') || normalized.includes('processing')) {
        return ('A nota ainda esta em processamento na Focus. Aguarde a conciliacao e atualize o status antes de reenviar.' +
            detailSuffix);
    }
    if (normalized.includes('aliquot') ||
        ...
        return ('Regra de aliquota do ISS incompativel com a retencao e o regime. ' +
    ...
    if (normalized.includes('inss') ||
        ...
        return ('A Focus rejeitou valores de retencoes (INSS/CP, federais ou congeneres) ou regras incompativeis. ' +
            'Revise retencao de INSS, base e valores com o contador, e a consistencia de valor liquido x ISS/INSS.' +
            detailSuffix);
    }
    if (normalized.includes('autentic') ||
        ...
        return ('Falha de autenticacao, token ou habilitacao de ambiente na Focus. ' +
    ...
    if (normalized.includes('ibge') || (normalized.includes('municipio') && normalized.includes('nao encont'))) {
        return ('Cidade, UF ou codigo de municipio (IBGE) nao puderam ser resolvidos para a Focus. ' +
    ...
    if (normalized.includes('time') ||
        ...
        return ('Houve falha de rede ou o servico da Focus nao respondeu a tempo. ' +
            'Aguarde e consulte a nota; evite reenvio duplicado no mesmo id/referencia.' +
            detailSuffix);
    }
    if (normalized.includes('competenc') ||
        normalized.includes('data de') ||
        normalized.includes('nao e permitid') && normalized.includes('data')) {
        return ('A Focus rejeitou data de emissao ou de competencia. Ajuste competencia, fuso/horario e o calendario fiscal do municipio.' +
            detailSuffix);
    }
    if (normalized.includes('schema') ||
        ...
        return ('A Focus rejeitou o layout da NFS-e por combinacao invalida de campos. ' +
            'Revise a preparacao da nota conforme a rota municipal ou nacional da empresa.' +
            detailSuffix);
    }
    return '';
}
function extractFocusRejectionErrorCode(data, messageForUser) {
    const fromUser = (messageForUser || '').match(/\bE\d{4}\b/i);
    if (fromUser) {
        return fromUser[0].toUpperCase();
    }
    const detailStr = focusProviderErrorDetailsText(data?.erros ?? data?.errors ?? data?.detalhes ?? data?.details);
    const fromDetails = (detailStr || '').match(/\bE\d{4}\b/i);
    if (fromDetails) {
        return fromDetails[0].toUpperCase();
    }
    const rawCode = asTrimmedString(data.codigo) ||
        asTrimmedString(data.code);
...
function providerIsFocus(provider) {
...
function focusNfseApiMode(setup) {
    const explicit = normalizeFocusText(setup.focusNfseApi) ||
        normalizeFocusText(setup.nfseApiMode) ||
        normalizeFocusText(setup.focusEmissionMode);
    if (explicit.includes('nacional') ||
...
function focusProviderLabel(setup) {
    return focusNfseApiMode(setup) === 'national'
        ? 'Focus NFe Nacional'
        : 'Focus NFe';
}
function normalizeOfficialInvoiceStatus(rawStatus, provider, fallback = 'DRAFT') {
...
function focusServerBase(environment) {
    return environment.trim().toLowerCase() === 'producao'
        ? 'https://api.focusnfe.com.br'
        : 'https://homologacao.focusnfe.com.br';
}
function focusAuthHeader(token) {
...
async function focusRequest(params) {
    ...
    if (!response.ok && response.status != 422 && response.status != 400) {
        const message = asTrimmedString(data.mensagem) ||
            asTrimmedString(data.message) ||
            asTrimmedString(data.erro) ||
            rawText.trim();
 ...
 */
function resolveFocusNationalTaxRegimeForSimples(invoiceTaxRegime, emitterTaxRegime) {
    ...
    const looksPlaceholder = invNorm.includes('validar com contador') ||
        invNorm === 'a definir' ||
        invNorm === '-' ||
...
function readFocusNationalSimplesNacionalOverride(tax) {
    const v = tax
        .opcaoSimplesNacional ?? tax
...
function taxRegimeTextForFocusSimpleOverride(code) {
    if (code === 2) {
        return 'mei';
    }
    if (code === 3) {
 ...
 */
function buildFocusInvoiceServiceDescription(data, service, emitter) {
    const baseServiceDesc = asTrimmedString(data
        .fiscalServiceBaseDescription) || asTrimmedString(service.description || data.serviceDescription);
    const serviceComplement = asTrimmedString(data
        .serviceDescriptionComplement);
    const serviceBodyNoBank = [baseServiceDesc, serviceComplement]
        .filter((s) => s.length > 0)
        .join('\n\n');
    const paymentBank = asTrimmedString(emitter.fiscalPaymentBankInfo);
    return paymentBank
        ? serviceBodyNoBank
            ? `${serviceBodyNoBank}\n\n${paymentBank}`
            : paymentBank
...
function focusNationalSimpleOptionCode(taxRegime) {
...
function focusNationalSimpleTaxRegimeCode(taxRegime) {
    const normalized = normalizeFocusText(taxRegime);
    if (normalized.includes('simples')) {
        return 1;
    }
...
function normalizeFocusNationalRetainedIssRate(params) {
    ...
    const requiresNationalMinimumRate = params.issRetained &&
        simpleOptionCode === 3 &&
...
async function resolveFocusMunicipalityCode(params) {
    ...
    if (Array.isArray(raw)) {
        ...
        if (first && typeof first === 'object') {
            return asTrimmedString(first.codigo_municipio);
        }
    }
    return '';
}
function focusRegimeTributario(companyData) {
    const regime = asTrimmedString(companyData.regimeTributario).toLowerCase();
    if (regime.includes('simples') || regime.includes('mei'))
        return '1';
    if (regime.includes('lucro presumido'))
        return '2';
    return '3';
}
function companyIsServiceBusiness(companyData) {
...
function inferCompanyTaxRegime(companyData) {
...
function classifyDirectSignupCompany(params) {
    ...
    const isMei = legalNatureSignals.some((item) => legalNature.includes(item)) ||
    ...
    if (isMei) {
        return {
            ...
            businessTier: 'mei',
            seatsIncluded: 1,
            reason: 'Classificacao oficial do CNPJ indica MEI. O sistema aplica fluxo Solo com 1 acesso base.',
            validationLabel: 'Validacao oficial: natureza juridica e porte retornados na consulta do CNPJ.',
        };
    }
    return {
        companyType: 'EMPRESA',
        ...
        validationLabel: 'Validacao oficial: natureza juridica, porte e atividade principal retornados na consulta do CNPJ.',
    };
}
function buildEmitterPayloadFromCompanyData(companyData) {
    const legalName = asTrimmedString(companyData.razaoSocial) ||
    ...
    const municipalRegistration = sanitizeMunicipalRegistrationFromCnpj(companyData.inscricaoMunicipal, companyData);
...
async function buildFocusCompanyPayload(params) {
    ...
    const municipalityCode = asTrimmedString(companyData.codigoMunicipio) ||
    ...
    const payload = {
        ...
        inscricao_municipal: sanitizeMunicipalRegistrationFromCnpj(companyData.inscricaoMunicipal, companyData),
...
async function downloadCertificateBase64(settingsData) {
    const certificate = asRecord(settingsData.fiscalCertificate);
...
function validateInvoiceReadinessForOfficialIssue(params) {
    ...
    if (!asTrimmedString(emitter.city))
        missing.push('cidade do emitente');
    if (!asTrimmedString(emitter.state))
        missing.push('UF do emitente');
    if (providerIsFocus(provider)) {
    ...
    if (!asTrimmedString(customer.city))
        missing.push('cidade do tomador');
    if (!asTrimmedString(customer.state))
        missing.push('UF do tomador');
    const amountCents = Number(data.amountCents ?? service.grossAmountCents ?? 0);
    if (!(amountCents > 0))
        missing.push('valor da nota');
    ...
    const nationalIssCodeForChecks = focusApiMode === 'national'
    ...
    if (focusApiMode === 'national' && nationalIssCodeForChecks) {
        ...
        const requiresMinRetainedIss = issRetained &&
    ...
    if (missing.length > 0) {
        throw new functions.https.HttpsError('failed-precondition', `Emissao oficial bloqueada. Ajuste: ${missing.join(', ')}.`);
    }
}
function normalizeInvoiceRefreshAttemptStatus(status) {
...
async function persistOfficialInvoiceStatus(params) {
    ...
    const rawStatus = coalesceFocusNfseStatusRawForNormalize(providerMap) ||
        asTrimmedString(params.providerResponse.status);
    const status = asUpperStatus(normalizeOfficialInvoiceStatus(rawStatus, params.providerResponse.provider, asTrimmedString(params.invoiceData.status) || 'DRAFT'), asTrimmedString(params.invoiceData.status) || 'DRAFT');
    const officialNumber = extractFocusOfficialNfseNumber(providerMap) ||
    ...
    const officialPortalUrl = asTrimmedString(params.providerResponse.officialPortalUrl) ||
    ...
    const protocol = asTrimmedString(params.providerResponse.protocol) ||
...
function buildFiscalOperationalPendingItems(params) {
    ...
    const pushItem = (input) => {
        ...
        items.push({
            ...
            owner: asTrimmedString(control.owner) ||
    ...
    if (!asTrimmedString(setup.municipalCode)) {
        pushItem({
            ...
            description: asTrimmedString(setup.focusNfseApi).toLowerCase() === 'national'
    ...
    for (const missing of provisioningMissing) {
        ...
        if (items.some((item) => asTrimmedString(item.code) === code))
            continue;
        pushItem({
            code,
...
function buildFiscalPlatformSnapshot(params) {
    ...
    const settingsData = params.settingsData;
    const companyData = mapCompanyData(settingsData.companyData) ||
    ...
    const criticalPending = pendingItems.filter((item) => asTrimmedString(item.severity) === 'critical').length;
    const documentPending = pendingItems.filter((item) => item.documentRequired === true).length;
    ...
    const overallStatus = asTrimmedString(provisioning.status) === 'ERROR'
        ? 'ERROR'
...
async function syncPlatformCompanyDataPatch(params) {
    const patchEntries = Object.entries(params.companyDataPatch).filter(([, value]) => value !== undefined);
...
async function validateInvoiceSourceTaskConsistency(params) {
    ...
    const sourceTaskId = asTrimmedString(sourceTask.id) ||
    ...
    if (!taskSnap.exists) {
        throw new functions.https.HttpsError('failed-precondition', 'A tarefa vinculada a nota nao foi encontrada.');
    }
    ...
    const invoiceCustomerDocument = onlyDigits(invoiceCustomer.document || params.invoiceData.clientDocument);
    if (taskCustomerId &&
        invoiceCustomerId &&
        taskCustomerId !== invoiceCustomerId) {
        throw new functions.https.HttpsError('failed-precondition', 'A tarefa vinculada pertence a um cliente diferente do tomador da nota.');
    }
    if (taskCustomerDocument &&
        invoiceCustomerDocument &&
        taskCustomerDocument !== invoiceCustomerDocument) {
        throw new functions.https.HttpsError('failed-precondition', 'O documento do cliente da tarefa nao confere com o tomador da nota.');
    }
}
async function ensureFinanceMovementForInvoice(params) {
    ...
    const issueDate = issueDateRaw instanceof admin.firestore.Timestamp
        ? issueDateRaw.toDate()
        : new Date();
    const dueDate = serviceDateRaw instanceof admin.firestore.Timestamp
        ? serviceDateRaw.toDate()
        : issueDate;
    const clientName = asTrimmedString(params.invoiceData.clientName) || 'Cliente';
    const serviceDescription = asTrimmedString(params.invoiceData.serviceDescription) || 'Servico';
    ...
    await movementRef.set({
        ...
        notes: `Origem fiscal: nota ${params.invoiceId} | Servico: ${serviceDescription}` +
...
function paymentFinanceMovementDocId(paymentId) {
    return `payment_${paymentId}`;
}
function debtFinanceMovementDocId(debtId) {
    return `debt_${debtId}`;
}
function paymentMovementFinancialStatus(status) {
...
function paymentMovementDateFromData(data) {
    const paidAt = data.paidAt;
...
async function syncFinanceMovementForPayment(params) {
    ...
    const competenceLabel = competenceYear > 0 && competenceMonth > 0
    ...
    await movementRef.set({
        ...
        dueDate: dueDateRaw instanceof admin.firestore.Timestamp
            ? dueDateRaw
            : null,
        paymentStatus: paymentMovementFinancialStatus(status),
        notes: `Origem trabalhista: pagamento ${params.paymentId}` +
...
function debtMovementShape(type) {
...
async function syncFinanceMovementForDebt(params) {
    ...
    const movementDate = debt.settledAt instanceof admin.firestore.Timestamp
    ...
    await movementRef.set({
        ...
        notes: `Origem obrigacao interna: ${params.debtId}` +
            ` | Colaborador: ${employeeId}` +
            ` | Tipo: ${type}`,
        sourceModule: 'debts',
...
async function buildFocusInvoicePayload(params) {
    ...
    const issueDate = issueDateRaw instanceof admin.firestore.Timestamp
        ? issueDateRaw.toDate()
        : new Date();
    const companyMunicipalityCode = asTrimmedString(emitter.codigoMunicipio) ||
        (await resolveFocusMunicipalityCode({
            token: params.token,
            environment: params.environment,
    ...
    const customerMunicipalityCode = (await resolveFocusMunicipalityCode({
        token: params.token,
        environment: params.environment,
    ...
    return {
        ...
        optante_simples_nacional: taxRegime.includes('simples') || taxRegime.includes('mei'),
        status: '1',
        prestador: {
        ...
        servico: {
            ...
            discriminacao: buildFocusInvoiceServiceDescription(asRecord(data), service, emitter),
...
function tryParseNationalLc116SixDigits(input) {
...
function formatNationalLc116Dotted(sixDigits) {
 ...
 */
function alignNationalServiceCodesOnInvoiceService(service) {
    const s = tryParseNationalLc116SixDigits(asTrimmedString(service.serviceCode));
    const m = tryParseNationalLc116SixDigits(asTrimmedString(service.municipalServiceCode));
    const pick = s || m;
    if (pick) {
    ...
    const natPref = onlyDigits(String(service.codigoTributacaoNacionalIss ||
...
function focusNationalTaxCode(params) {
    ...
    const preferred = onlyDigits(service.codigoTributacaoNacionalIss ||
        service.nationalTaxCode ||
        service.nationalServiceCode ||
        service.nationalIssTaxCode);
    ...
    const description = slugifyFiscalText(service.description || params.invoiceData.serviceDescription);
    if (cnae === '4321500') {
        if (description.includes('instal'))
            return '070202';
        ...
        const isPredial = description.includes('predial') ||
        ...
        if (description.includes('manut') && isPredial)
            return '070501';
    }
    if (cnae.length === 7) {
...
async function buildFocusNationalInvoicePayload(params) {
    ...
    const issueDate = issueDateRaw instanceof admin.firestore.Timestamp
        ? issueDateRaw.toDate()
        : new Date();
    const serviceDate = serviceDateRaw instanceof admin.firestore.Timestamp
        ? serviceDateRaw.toDate()
        : issueDate;
    const companyMunicipalityCode = asTrimmedString(emitter.codigoMunicipio) ||
        (await resolveFocusMunicipalityCode({
    ...
    if (!municipalityPattern.test(companyMunicipalityCode)) {
        throw new functions.https.HttpsError('failed-precondition', `Nao foi possivel resolver o codigo IBGE (7 digitos) do municipio do emitente. Cidade/UF: "${asTrimmedString(emitter.city)}" / "${asTrimmedString(emitter.state)}".`);
    }
    if (!municipalityPattern.test(customerMunicipalityCode)) {
        throw new functions.https.HttpsError('failed-precondition', `Nao foi possivel resolver o codigo IBGE (7 digitos) do municipio do tomador. Cidade/UF: "${asTrimmedString(customer.city)}" / "${asTrimmedString(customer.state)}".`);
    }
    const customerDocument = onlyDigits(customer.document);
    ...
    const hasAnyCustomerAddressPiece = customerZip.length > 0 ||
    ...
    if (hasAnyCustomerAddressPiece && customerNumber.length === 0) {
        throw new functions.https.HttpsError('failed-precondition', 'Endereco do tomador incompleto: informe o NUMERO do tomador (campo "Numero") para emitir no padrao nacional.');
    }
    const amountCents = Number(data.amountCents ?? service.grossAmountCents ?? 0);
    const deductionsCents = Number(tax.deductionsCents ?? 0);
    const taxableBaseCents = Number(tax.taxableBaseCents ?? Math.max(amountCents - deductionsCents, 0));
    ...
    const rawIssRate = asFocusDecimal(tax.issRate);
    const taxRegime = asTrimmedString(resolveFocusNationalTaxRegimeForSimples(asTrimmedString(tax.taxRegime), asTrimmedString(emitter.taxRegime))).toLowerCase();
    ...
    const workSiteCno = onlyDigits(workSite.cno || workSite.cno_obra || workSite.cnoObra || '');
    ...
    if (requiresObraGroup && workSiteCno.length === 0) {
        throw new functions.https.HttpsError('failed-precondition', `Para NFSe Nacional com codigo de tributacao nacional ${nationalTaxCode}, o CNO da obra e obrigatorio (grupo de obra).`);
    }
    const nationalSimpleTaxRegimeCode = focusNationalSimpleTaxRegimeCode(taxRegimeForFocusRules);
    const simpleOptionCode = simplesOverride ?? focusNationalSimpleOptionCode(taxRegimeForFocusRules);
    const competenceDate = formatSaoPauloDate(serviceDate);
    const municipalityServiceCode = onlyDigits(asTrimmedString(service.municipalServiceCode));
    const municipalTaxCode = municipalityServiceCode.length === 3 ? municipalityServiceCode : '';
    const customerLegalName = asTrimmedString(customer.legalName || data.clientName);
    const workSiteMunicipalityCode = knownMunicipalityIbgeCodeByLocation(workSite.city, workSite.state) ||
        companyMunicipalityCode;
    const serviceMunicipalityCode = asTrimmedString(workSite.zipCode) && workSiteMunicipalityCode
        ? workSiteMunicipalityCode
        : companyMunicipalityCode;
    if (!municipalityPattern.test(serviceMunicipalityCode)) {
        throw new functions.https.HttpsError('failed-precondition', `Nao foi possivel resolver o codigo IBGE (7 digitos) do municipio de prestacao. Emitente: "${asTrimmedString(emitter.city)}" / "${asTrimmedString(emitter.state)}". Obra: "${asTrimmedString(workSite.city)}" / "${asTrimmedString(workSite.state)}".`);
    }
    const totalMunicipalTributes = (issAmountCents / 100).toFixed(2);
    const fullServiceDescription = buildFocusInvoiceServiceDescription(asRecord(data), service, emitter);
    ...
    const requiresRetainedIssAliquot = issRetained && simpleOptionCode === 3 && nationalSimpleTaxRegimeCode === 1;
    const forbidsIssAliquotWithoutRetention = !issRetained && simpleOptionCode === 3 && nationalSimpleTaxRegimeCode === 1;
    if (requiresRetainedIssAliquot && issRate < 1.8) {
        throw new functions.https.HttpsError('failed-precondition', 'A NFS-e Nacional exige aliquota minima de 1,80% para Simples Nacional com ISS retido.');
    }
    return {
        referencia: params.invoiceId,
        ...
        ...(() => {
            ...
            const hasAnyAddressPiece = zip.length > 0 || street.length > 0 || number.length > 0 || neighborhood.length > 0;
            ...
            if (hasAnyAddressPiece && zip.length > 0 && number.length === 0) {
                throw new functions.https.HttpsError('failed-precondition', 'Endereco da obra incompleto: informe o NUMERO da obra (campo "Numero") ou remova CEP/endereco da obra e envie apenas o CNO.');
            }
...
async function issueWithFocus(params) {
    ...
    const payload = apiMode === 'national'
    ...
    if (apiMode === 'national') {
        delete payload.codigo_tributacao_municipal_iss;
        delete payload.codigo_tributario_municipio;
        const nationalCode = asTrimmedString(payload.codigo_tributacao_nacional_iss);
        const fiscalPayloadSummary = {
            codigo_tributacao_nacional_iss: nationalCode,
            codigo_tributacao_municipal_iss: payload.codigo_tributacao_municipal_iss,
            codigo_tributario_municipio: payload.codigo_tributario_municipio,
            codigo_opcao_simples_nacional: payload.codigo_opcao_simples_nacional,
            regime_tributario_simples_nacional: payload.regime_tributario_simples_nacional,
            tipo_retencao_iss: payload.tipo_retencao_iss,
            percentual_aliquota_relativa_municipio: payload.percentual_aliquota_relativa_municipio,
            aliquota: payload.aliquota,
            base_calculo: payload.base_calculo,
            valor_iss: payload.valor_iss,
            valor_iss_retido: payload.valor_iss_retido,
            valor_cp: payload.valor_cp,
        ...
        functions.logger.info('focus national payload summary', {
            ...
            has_cno_obra: Boolean(payload.cno_obra),
            cno_obra_len: asTrimmedString(payload.cno_obra).length,
        });
    }
    ...
    if (createResponse.status >= 400) {
        const errMsg = focusProviderErrorMessage(createResponse.data, 'A Focus rejeitou a solicitacao de emissao da nota.');
    ...
    for (let attempt = 0; attempt < 5; attempt += 1) {
        ...
        const status = normalizeOfficialInvoiceStatus(rawStatus || latestMap.status, focusProviderLabel(params.setup), 'PROCESSING');
...
async function callFiscalProvider(params) {
    ...
    if (!apiBaseUrl) {
        throw new functions.https.HttpsError('failed-precondition', 'Base URL da integracao fiscal nao configurada.');
    }
    ...
    try {
        ...
        throw new functions.https.HttpsError('unavailable', `Falha ao conectar no endpoint fiscal: ${String(error)}`);
    }
    const rawText = await response.text();
    let parsed = {};
    if (rawText.trim().length > 0) {
    ...
    if (!response.ok) {
        const providerMessage = asTrimmedString(parsed.message) ||
            asTrimmedString(parsed.error) ||
            rawText.trim();
        throw new functions.https.HttpsError('internal', providerMessage.length > 0
            ? providerMessage
...
function ensureEmployeeOwner(paymentOrDebtEmployeeId, claims) {
...
function knownMunicipalityIbgeCodeByLocation(cityValue, stateValue) {
    ...
    if (city === 'hidrolandia' && (state === 'go' || state === 'goias'))
        return '5209705';
    // Hidrolândia-CE (IBGE)
    if (city === 'hidrolandia' && (state === 'ce' || state === 'ceara'))
        return '2305230';
    ...
    if (city === 'brasilia' && isDf)
        return '5300108';
    if (city === 'goiania' && state === 'go')
        return '5208707';
    return '';
}
function asTrimmedRecord(value) {
    return asRecord(value);
}
function companyIbgeCode(companyData) {
    const explicit = asTrimmedString(companyData.codigoMunicipio ||
        companyData.cityCode ||
        companyData.ibgeCode ||
        companyData.codigoIbge);
    if (explicit)
        return explicit;
    return knownMunicipalityIbgeCodeByLocation(companyData.cidade || companyData.city || companyData.municipio, companyData.estado || companyData.state || companyData.uf);
}
/** Prestador: app historico gravava apenas municipalRegistration no emitter. */
function emitterInscricaoMunicipal(emitter) {
    const direct = asTrimmedString(emitter.inscricaoMunicipal);
    if (direct)
        return direct;
    return asTrimmedString(emitter.municipalRegistration);
}
function isKnownNationalNfseMunicipality(companyData) {
    ...
    return ((city === 'hidrolandia' && state === 'go') ||
        ibge === '5209705');
}
async function resolveFiscalMunicipalityRoute(companyData) {
    const ibge = companyIbgeCode(companyData);
    const city = asTrimmedString(companyData.cidade || companyData.city || companyData.municipio);
    ...
    if (isKnownNationalNfseMunicipality(companyData)) {
        return {
            ...
            detectionReason: 'Municipio identificado como aderente ao ambiente nacional da NFSe.',
        };
    }
    return {
        found: false,
...
async function buildRecommendedFiscalSetup(params) {
    ...
    const focusApi = asTrimmedString(municipalRoute.focusNfseApi) ||
        (routeType === 'focus_national' ? 'national' : 'municipal');
    const recommendedProvider = asTrimmedString(municipalRoute.provider) ||
        (serviceBusiness ? 'Focus NFe' : 'Prefeitura direta');
    const requiresManualReview = !city ||
        !state ||
        routeType === 'manual_review' ||
        currentRouting.manualOverride === true;
    const municipalCode = asTrimmedString(currentIntegration.municipalCode) ||
    ...
    const routing = {
        ...
        detectionReason: asTrimmedString(municipalRoute.detectionReason) ||
            (serviceBusiness
        ...
        autoDetectionSource: asTrimmedString(municipalRoute.source) || 'backend_auto_setup',
    };
    const realIntegration = {
        environment: asTrimmedString(currentIntegration.environment) || platformFocus.environment || 'homologacao',
        ...
        apiBaseUrl: asTrimmedString(currentIntegration.apiBaseUrl) ||
...
async function autoProvisionFocusCompanyIfReady(params) {
    ...
    try {
        ...
        const message = errorMessage(error, 'Falha ao provisionar automaticamente a empresa na Focus.');
        await settingsRef.set({
            focusProvisioning: {
                status: 'ERROR',
                missing: [],
...
async function mergeFiscalSecureSettings(companyId, settingsData) {
...
function formatBrazilCnpjForDisplay(value) {
    const d = onlyDigits(value);
    if (d.length !== 14)
        return '';
    return `${d.slice(0, 2)}.${d.slice(2, 5)}.${d.slice(5, 8)}/${d.slice(8, 12)}-${d.slice(12, 14)}`;
}
function buildCompanyDisplayCode(params) {
    const digits = onlyDigits(params.cnpj).padEnd(5, '0').slice(0, 5);
    const firstWord = String(params.companyName ?? '')
        .trim()
        .split(/\s+/)
...
function toAsaasMoneyValue(valueCents) {
    ...
    if (!Number.isFinite(cents) || cents <= 0) {
        throw new functions.https.HttpsError('invalid-argument', 'Valor mensal invalido para provisionar no Asaas.');
    }
    return Math.round(cents) / 100;
}
function formatCurrencyBr(valueCents) {
    const cents = Number(valueCents ?? 0);
...
function asaasSubscriptionCycle(value) {
    const normalized = asTrimmedString(value).toUpperCase();
...
function asaasBillingType(value) {
    const normalized = asTrimmedString(value).toUpperCase();
    if (normalized === 'PIX')
        return 'PIX';
...
function buildAsaasCustomerPayload(params) {
    ...
    const name = asTrimmedString(companyData.nomeFantasia) ||
        asTrimmedString(companyData.razaoSocial) ||
    ...
    if (asTrimmedString(companyData.endereco))
        payload.address = asTrimmedString(companyData.endereco);
    if (asTrimmedString(companyData.numero))
        payload.addressNumber = asTrimmedString(companyData.numero);
    if (asTrimmedString(companyData.complemento))
        payload.complement = asTrimmedString(companyData.complemento);
    if (asTrimmedString(companyData.bairro))
        payload.province = asTrimmedString(companyData.bairro);
    if (postalCode)
        payload.postalCode = postalCode;
    return payload;
}
/** CNAE pode vir como string, numero, ou em subobjetos (Brasil API / cache). */
function cnaeDigitsFromBrasilLikeSource(source) {
    if (source == null)
        return '';
    ...
    const candidates = [
        ...
        r.atividade_principal?.id,
        r.atividade_principal?.code,
        r.estabelecimento?.cnae_fiscal,
    ];
 ...
 */
function sanitizeMunicipalRegistrationFromCnpj(municipalRaw, cnaeSource) {
    const trimmed = String(municipalRaw ?? '').trim();
    if (!trimmed)
        return '';
    const im = onlyDigits(trimmed);
    const cnae = typeof cnaeSource === 'object' && cnaeSource !== null && !Array.isArray(cnaeSource)
    ...
    if (im === cnae)
        return '';
    if (im.length >= 5 && im.substring(0, 5) === prefix5)
        return '';
    return trimmed;
}
/** Cache antigo pode ter IM = prefixo de CNAE; re-sanitiza ao devolver. */
function applyCnpjPayloadSanitizedFields(payload) {
    return {
        ...payload,
        municipalRegistration: sanitizeMunicipalRegistrationFromCnpj(payload.municipalRegistration, payload),
    };
}
function mapCnpjPayload(raw) {
    const partners = Array.isArray(raw?.qsa)
        ? raw.qsa
            .map((item) => String(item?.nome_socio ?? '').trim())
            .filter(Boolean)
        : [];
    const secondaryCnaes = Array.isArray(raw?.cnaes_secundarios)
        ? raw.cnaes_secundarios
            ...
            return {
                ...
                description: String(item?.descricao ?? item?.description ?? item?.text ?? '').trim(),
            };
        })
            .filter(Boolean)
        : [];
    return {
        ...
        municipalRegistration: sanitizeMunicipalRegistrationFromCnpj(raw?.inscricao_municipal, raw),
...
function mapCnpjWsPayload(raw) {
    ...
    const stateRegistration = Array.isArray(establishment?.inscricoes_estaduais)
        ? establishment.inscricoes_estaduais
            .map((item) => String(item?.inscricao_estadual ?? '').trim())
            .find(Boolean) ?? ''
        : '';
    ...
    return {
        ...
        phone: String(establishment?.telefone1 ??
        ...
        stateRegistration,
        municipalRegistration: '',
        mainCnae: String(mainActivity?.id ??
            mainActivity?.subclasse ??
            establishment?.cnae_fiscal ??
            '').trim(),
        mainCnaeDescription: String(mainActivity?.descricao ?? establishment?.atividade_principal?.descricao ?? '').trim(),
...
async function fetchCnpjPayload(cnpj) {
    ...
    throw new functions.https.HttpsError('not-found', 'Nao foi possivel localizar dados para este CNPJ.');
}
function mapCepPayload(raw) {
    return {
        found: true,
        source: 'viacep',
        zipCode: onlyDigits(raw?.cep),
...
async function readRegistryCache(key) {
    const snap = await admin.firestore().collection('registry_cache').doc(key).get();
    if (!snap.exists)
        return null;
    return snap.data() ?? null;
}
async function writeRegistryCache(key, payload) {
...
const OFFICE_FOUNDER_OPERATING_COMPANY_NAME = 'BONFIM AUTOMAÇÃO E INSTALAÇÕES ELÉTRICAS LTDA';
...
function officeFounderOperatingCompanyCredibilityHtml() {
    return `
...
function officeFounderEmailSignOffInnerHtml() {
...
function officeFounderFormalClosingInviteHtml() {
    return `<p style="margin: 24px 0 0 0; line-height: 1.55;">Fico à disposição.</p>
      <p style="margin: 16px 0 0 0; line-height: 1.55; color: #111;">&mdash;<br/>${escapeHtml(OFFICE_FOUNDER_DISPLAY_NAME)}<br/><span style="color:#374151;">Fundador | Ponto Certo</span></p>`;
}
function trialInviteTransparencySectionHtml(params) {
    ...
    const parts = [];
    parts.push(`<p style="margin: 0 0 12px 0;"><strong>Sobre a empresa referenciada neste convite:</strong></p>`);
    parts.push(`<div style="margin: 0 0 20px 0; padding: 16px 18px; background: #f8fafc; border-left: 4px solid #2563eb; border-radius: 0 8px 8px 0; line-height: 1.5;">`);
    if (companyName) {
        parts.push(`<p style="margin: 0 0 8px 0;"><strong>Empresa:</strong> ${escapeHtml(companyName)}</p>`);
    }
    if (cnpjFormatted) {
        parts.push(`<p style="margin: 0 0 8px 0;"><strong>CNPJ:</strong> ${escapeHtml(cnpjFormatted)}</p>`);
    }
    if (openedAtLabel) {
        parts.push(`<p style="margin: 0 0 8px 0;"><strong>Abertura:</strong> ${escapeHtml(openedAtLabel)}</p>`);
    }
    if (companyEmail && !companyName) {
        parts.push(`<p style="margin: 0;"><strong>E-mail:</strong> ${escapeHtml(companyEmail)}</p>`);
    }
    parts.push(`</div>`);
    return parts.join('');
}
...
function officeFounderPresentationHtml() {
    return `
      <hr style="border:none; border-top:1px solid #e5e7eb; margin:24px 0;" />
      <p style="margin:0 0 10px 0;"><strong>Quem esta por tras do Ponto Certo</strong></p>
      <p>Sou <strong>${escapeHtml(OFFICE_FOUNDER_DISPLAY_NAME)}</strong>, fundador e desenvolvedor do produto, e empresario nos segmentos de obras, construcao civil e servicos eletricos. O Ponto Certo nasceu na pratica: para reduzir retrabalho na emissao de notas, dar previsibilidade ao financeiro e fortalecer a parceria entre empresa e escritorio contabil.</p>
...
async function enviarEmailPreCadastroPlano(params) {
    ...
    <div style="font-family: Arial, Helvetica, sans-serif; color: #111; line-height: 1.5;">
      ...
      <p>${params.implementationMode === 'accountant'
        ...
        : 'Este fluxo segue com contador como contato principal para iniciar o escritorio e depois cadastrar a empresa.'}</p>
...
async function ensureCompanyOwnerAccess(params) {
    ...
    if (!ownerEmail) {
        throw new functions.https.HttpsError('invalid-argument', 'Email de acesso da empresa obrigatorio.');
    }
    let userRecord = null;
...
async function enviarNotificacaoInternaNovoCadastro(params) {
    try {
        ...
        const toInternal = fromRaw.includes('@') && !fromRaw.startsWith('@') ? fromRaw : PLATFORM_SIGNUP_NOTIFICATION_EMAIL;
        const fromEmail = fromRaw.includes('@') && !fromRaw.startsWith('@')
            ? fromRaw
...
async function notificarNovoCadastroAdministrativo(params) {
    ...
    const subject = params.companyName || params.officeName
...
async function provisionLightweightOfficeAccess(params) {
    ...
    await accountingOfficeRef().doc(officeId).set(omitUndefinedForFirestore({
        officeId,
        officeName,
        cnpj: '',
        responsibleName,
    ...
    await admin.firestore().collection('users').doc(userRecord.uid).set(omitUndefinedForFirestore({
        companyId: officeId,
...
async function createOrUpdateAccountingOffice(params) {
    ...
    if (!existingOfficeSnap.empty) {
        ...
        if (existingEmail && existingEmail !== email) {
            throw new functions.https.HttpsError('already-exists', 'Ja existe escritorio cadastrado com este CNPJ usando outro email.');
        }
    }
    let userRecord = null;
    let createdNow = false;
    ...
    if (!userRecord) {
        ...
        if (existingUserSnap.exists) {
            ...
            if (existingRole !== 'ACCOUNTANT') {
                throw new functions.https.HttpsError('already-exists', 'O email informado ja esta em uso por um acesso que nao e de escritorio contabil.');
            }
        }
        await admin.auth().updateUser(userRecord.uid, {
            password,
    ...
    const officeId = !existingOfficeSnap.empty ? existingOfficeSnap.docs[0].id : `office_${Date.now()}`;
...
async function createImplementationChargeForOnboarding(params) {
    ...
    const implementationFeeCents = params.implementationFeeCents == null
        ? Number(item.implementationFeeCents ?? 0) || 0
        : params.implementationFeeCents;
    if (!customerId || implementationFeeCents <= 0)
        return null;
    ...
    if (asTrimmedString(existingCharge.paymentId)) {
        return {
            ...
            invoiceUrl: asTrimmedString(existingCharge.invoiceUrl) ||
    ...
    const dueDate = admin.firestore.Timestamp.fromMillis(Date.now() + 30 * 24 * 60 * 60 * 1000);
...
async function createOrRefreshPlanUpgradeCharge(params) {
    ...
    if (amountCents <= 0) {
        throw new functions.https.HttpsError('failed-precondition', 'A taxa de aquisicao para migrar ao plano Equipe nao esta configurada.');
    }
    ...
    const existingPaymentLinkUrl = asTrimmedString(params.currentPlanUpgrade.paymentLinkUrl) ||
    ...
    if (!customerId) {
        throw new functions.https.HttpsError('failed-precondition', 'Nao foi possivel localizar o cliente Asaas da empresa para gerar a cobranca de upgrade.');
    }
    const dueDate = admin.firestore.Timestamp.fromMillis(Date.now() + 3 * 24 * 60 * 60 * 1000);
    ...
    return {
        paymentId: asTrimmedString(charge.id),
        paymentLinkUrl: asTrimmedString(charge.invoiceUrl) ||
...
async function finalizeSalesOnboardingToOperationalCompany(params) {
    ...
    const uploads = Array.isArray(item.uploads)
        ...
        : [];
    const certificateUpload = uploads.find((upload) => asTrimmedString(upload.category) === 'certificado_digital_a1') ?? null;
    const ownerEmail = asTrimmedString(formData.preferredLoginEmail).toLowerCase() ||
        asTrimmedString(formData.ownerEmail).toLowerCase();
    const ownerName = asTrimmedString(formData.ownerName) ||
        asTrimmedString(item.customerName) ||
        'Responsavel';
    const companyName = asTrimmedString(formData.tradeName) ||
        asTrimmedString(formData.legalName) ||
        asTrimmedString(item.planTitle) ||
        'Empresa';
    if (!ownerEmail) {
        throw new functions.https.HttpsError('failed-precondition', 'Onboarding sem email de login para criar a empresa operacional.');
    }
    ...
    const accountantName = asTrimmedString(item.accountantName) ||
        (asTrimmedString(item.implementationMode) === 'accountant'
            ? asTrimmedString(item.customerName)
            : '');
    const accountantEmail = asTrimmedString(item.accountantEmail).toLowerCase() ||
        (asTrimmedString(item.implementationMode) === 'accountant'
    ...
    if (accountantEmail) {
        ...
        if (accountantProfile && roleParaFirestore(accountantProfile.role) === 'ACCOUNTANT') {
            await admin.firestore().collection('accountant_links').doc(`${companyId}_${leadSnap.docs[0].id}`).set({
                ...
                accountantName: asTrimmedString(accountantProfile.nome) || accountantName,
                accountantEmail: asTrimmedString(accountantProfile.email).toLowerCase() || accountantEmail,
...
async function createOrRefreshPublicSalesOnboarding(params) {
    ...
    const customerEmail = asTrimmedString(customer.email) || asTrimmedString(params.payment.customerEmail);
    if (!customerEmail)
        return null;
...
async function provisionDirectSignupBilling(params) {
    ...
    if (!customerId) {
        throw new functions.https.HttpsError('internal', 'Asaas nao retornou customerId no cadastro direto.');
    }
...
async function resolveTrialConversionPlan(params) {
    ...
    const publicConfigSnap = await publicSalesConfigRef().get();
    const publicConfig = buildDefaultPublicSalesConfig(asRecord(publicConfigSnap.data()));
    const fallbackPlanCode = currentPlan === 'solo' || companyPlan === 'SOLO' ? 'solo' : 'equipe';
    const fallbackPlan = asRecord(fallbackPlanCode === 'solo' ? publicConfig.planSolo : publicConfig.planEquipe);
    ...
    return {
        ...
        monthlyPriceCents: currentPrice > 0 ? currentPrice : Number(fallbackPlan.priceCents ?? 0) || 0,
    };
}
async function ensureAccountantAccessForCompany(params) {
    ...
    if (userSnap.exists && existingRole !== 'ACCOUNTANT') {
        throw new functions.https.HttpsError('already-exists', 'O email informado ja esta em uso por um acesso que nao e de contador.');
    }
    const accountantClaimsCompanyId = asTrimmedString(existing.currentCompanyId) ||
        asTrimmedString(existing.companyId) ||
        params.companyId;
    await userRef.set({
        companyId: asTrimmedString(existing.companyId) || params.companyId,
        currentCompanyId: accountantClaimsCompanyId,
    ...
    const loginUrl = `https://gestao-ponto-certo.com/login-contador?email=${encodeURIComponent(accountantEmail)}&empresa=${encodeURIComponent(params.companyDisplayCode)}`;
...
function rawDateToMillis(value) {
    if (value instanceof admin.firestore.Timestamp)
        return value.toMillis();
    if (value instanceof Date)
...
async function resolveAccountantCompanyContext(userUid, preferredCompanyId) {
...
async function persistAccountantCurrentCompany(userUid, companyId) {
    if (!userUid.trim() || !companyId.trim())
        return;
...
exports.authSyncClaims = functions.https.onCall(async (_data, context) => {
    ...
    const preferredCompanyId = asTrimmedString(user.currentCompanyId) ||
    ...
    if (!companyId || !employeeId) {
        throw new functions.https.HttpsError('failed-precondition', 'Perfil sem companyId/employeeId para gerar custom claims.');
    }
...
exports.authSelectAccountantCompany = functions.https.onCall(async (data, context) => {
    ...
    const linkSnap = await admin
        ...
        .get();
    if (linkSnap.empty) {
        throw new functions.https.HttpsError('permission-denied', 'O contador nao possui vinculo ativo com a empresa informada.');
    }
...
exports.authSetClaimsForUser = functions.https.onCall(async (data, context) => {
    ...
    const companyId = role === 'ACCOUNTANT'
        ? await resolveAccountantCompanyContext(targetUid, asTrimmedString(target.currentCompanyId) ||
...
exports.platformListCompanies = functions.https.onCall(async (_data, context) => {
    ...
    const rows = await Promise.all(usersSnap.docs.map(async (doc) => {
        ...
        const accountantPendingStatus = accountantPendingStatusRaw === 'invited'
        ...
        const item = {
            ...
            companyDocument: asTrimmedString(asRecord(data.companyData).cnpj) ||
            ...
            contractedAppUsers: Number(commercial.contractedAppUsers ?? commercial.seatsIncluded ?? 3) || 3,
            baseSystemPriceCents: Number(commercial.baseSystemPriceCents ?? 0) || 0,
            extraAppUserPriceCents: Number(commercial.extraAppUserPriceCents ?? 0) || 0,
            monthlyPriceCents: Number(commercial.monthlyPriceCents ?? 0) || 0,
            calculatedMonthlyPriceCents: Number(commercial.calculatedMonthlyPriceCents ?? commercial.monthlyPriceCents ?? 0) || 0,
...
exports.platformUpdateCompanyFiscalStatus = functions.https.onCall(async (data, context) => {
    ...
    const integrationPatch = {
        environment: asTrimmedString(integrationPatchRaw.environment) || currentIntegration.environment,
        provider: asTrimmedString(integrationPatchRaw.provider) || currentIntegration.provider,
        focusNfseApi: asTrimmedString(integrationPatchRaw.focusNfseApi) || currentIntegration.focusNfseApi,
        municipalCode: asTrimmedString(integrationPatchRaw.municipalCode) || currentIntegration.municipalCode,
        certificateRef: asTrimmedString(integrationPatchRaw.certificateRef) || currentIntegration.certificateRef,
        apiBaseUrl: asTrimmedString(integrationPatchRaw.apiBaseUrl) || currentIntegration.apiBaseUrl,
        apiToken: asTrimmedString(integrationPatchRaw.apiToken) || currentIntegration.apiToken,
        lastHomologationNote: asTrimmedString(integrationPatchRaw.lastHomologationNote) ||
            currentIntegration.lastHomologationNote,
    };
    const checklistPatchRaw = asRecord(data?.checklistPatch);
    const checklistPatch = {
        companyBaseReviewed: checklistPatchRaw.companyBaseReviewed ?? currentChecklist.companyBaseReviewed,
        certificateValidated: checklistPatchRaw.certificateValidated ?? currentChecklist.certificateValidated,
        matrixValidated: checklistPatchRaw.matrixValidated ?? currentChecklist.matrixValidated,
        providerConnectionValidated: checklistPatchRaw.providerConnectionValidated ??
            currentChecklist.providerConnectionValidated,
        pilotInvoiceValidated: checklistPatchRaw.pilotInvoiceValidated ?? currentChecklist.pilotInvoiceValidated,
        productionAuthorized: checklistPatchRaw.productionAuthorized ?? currentChecklist.productionAuthorized,
    };
    const pendingItemsRaw = Array.isArray(data?.pendingItems) ? data.pendingItems : [];
    const nextPendingControl = { ...currentControl };
    ...
    const hasSecurePatch = asTrimmedString(securePatchRaw.certificatePassword) ||
    ...
    const companyData = mapCompanyData(refreshedSettingsData.companyData) ||
        mapCompanyData(ownerData.companyData);
    if (!companyData) {
        throw new functions.https.HttpsError('failed-precondition', 'Dados da empresa nao encontrados para atualizar a automacao fiscal.');
    }
    ...
    const finalSettingsData = await mergeFiscalSecureSettings(companyId, asRecord(finalSettingsSnap.data()));
    const snapshot = buildFiscalPlatformSnapshot({
        companyId,
        ownerUid: ownerDoc.id,
        ownerData,
    ...
    if (sendPendingEmail) {
        const pendingItems = snapshot.pendingItems.filter((item) => {
...
exports.platformSyncCompanyFocus = HEAVY_RUNTIME.https.onCall(async (data, context) => {
    ...
    const setup = asRecord(settingsData.fiscalRealIntegration);
    if (!providerIsFocus(asTrimmedString(setup.provider))) {
        throw new functions.https.HttpsError('failed-precondition', 'Selecione Focus NFe como provedor antes de sincronizar.');
    }
    const result = await syncFocusCompany({
        claims: {
            uid: claims.uid,
...
exports.platformExtendCompanyTrial = functions.https.onCall(async (data, context) => {
    ...
    if (lifecycleStatus !== 'trial') {
        throw new functions.https.HttpsError('failed-precondition', 'Esta empresa nao esta em modo trial.');
    }
    const now = Date.now();
    ...
    const baseMillis = Number.isFinite(currentGraceUntilMillis)
        ...
        : now;
    const nextGraceUntil = admin.firestore.Timestamp.fromMillis(baseMillis + extraDays * 24 * 60 * 60 * 1000);
    await settingsRef.set({
        commercialSettings: {
            ...currentCommercial,
            allowLogin: true,
            ...
            billingIntegration: {
                ...
                graceDays: Math.max(Number(currentBilling.graceDays ?? 0) || 0, Number(extraDays) || 0),
            },
...
exports.platformListTrialInvites = functions.https.onCall(async (data, context) => {
    ...
    const items = snap.docs.map((doc) => {
        ...
        const effectiveStatus = storedStatus === 'issued' &&
...
exports.platformDeleteTrialInvites = functions.https.onCall(async (data, context) => {
    ...
    const userProfile = await carregarUsuarioMesmoTenant(claims.uid, claims);
    assertPlatformAdmin(claims, userProfile);
    const rawInviteIds = Array.isArray(data?.inviteIds)
        ? data.inviteIds
        : [];
    const inviteIds = rawInviteIds
 ...
 */
async function listCompanySettingsLinkedToOffice(officeId) {
    const snap = await admin
        .firestore()
        .collection('company_settings')
    ...
    for (const doc of snap.docs) {
        ...
        out.push({
            companyId,
            companyName: asTrimmedString(companyData.nomeFantasia) ||
                asTrimmedString(companyData.razaoSocial) ||
                companyId,
...
exports.platformListAccountingOffices = functions.https.onCall(async (data, context) => {
    ...
    const items = await Promise.all(orderedDocs.map(async (doc) => {
        const o = asRecord(doc.data());
        const officeId = doc.id;
...
exports.platformSetAccountingOfficeAccess = functions.https.onCall(async (data, context) => {
    ...
    if (!allowAccess && !reason) {
        throw new functions.https.HttpsError('invalid-argument', 'Informe o motivo administrativo ao suspender acesso.');
    }
    const issuerName = asTrimmedString(userProfile.nome) || asTrimmedString(userProfile.name) || 'platform';
...
exports.platformReconcileAccountingOfficeTrialInvite = functions.https.onCall(async (data, context) => {
    ...
    for (const doc of trialSnap.docs) {
        ...
        if (status === 'used' && asTrimmedString(t.usedOfficeId) === '') {
            batch.set(doc.ref, { usedOfficeId: officeId, updatedAt: admin.firestore.FieldValue.serverTimestamp() }, { merge: true });
            updated += 1;
        }
...
exports.platformListSalesPipeline = functions.https.onCall(async (_data, context) => {
    ...
    const userProfile = await carregarUsuarioMesmoTenant(claims.uid, claims);
    assertPlatformAdmin(claims, userProfile);
    const [leadSnap, onboardingSnap, employeeTesterSnap, feedbackSnap, feedbackIncidentSnap, feedbackIssueSnap, unmatchedBillingSnap, pendingAccountantSnap, officeBillingSnap,] = await Promise.all([
    ...
    const onboardings = onboardingSnap.docs.map((doc) => {
        ...
        return {
            ...
            implementationChargeInvoiceUrl: asTrimmedString(implementationCharge.invoiceUrl) ||
    ...
    const testerLeadsByUid = new Map();
    ...
    const userProfiles = await Promise.all([...userIds].map(async (userId) => {
        ...
        return [userId, asRecord(snap.data())];
    }));
    const userProfilesById = new Map(userProfiles);
    const incidentsByFeedbackId = new Map();
    for (const doc of feedbackIncidentSnap.docs) {
        const item = asRecord(doc.data());
        const extra = asRecord(item.extra);
        const feedbackId = asTrimmedString(extra.feedbackId) ||
    ...
    const issuesByIncidentId = new Map();
    ...
    const productIdeas = feedbackSnap.docs.map((doc) => {
        ...
        const companyName = asTrimmedString(profile.companyName) ||
            asTrimmedString(companyData.nomeFantasia) ||
            asTrimmedString(companyData.razaoSocial) ||
            (companyId === PUBLIC_EMPLOYEE_TESTER_COMPANY_ID
        ...
        return {
            ...
            companyName,
            userId: testerUid,
            userName: asTrimmedString(lead.fullName) ||
            ...
            userRole: asTrimmedString(item.userRole) ||
            ...
            assistantSummary: asTrimmedString(incident.assistantSummary) ||
                asTrimmedString(issue.assistantSummary) ||
                asTrimmedString(item.assistantSummary),
            recommendedAction: asTrimmedString(incident.recommendedAction) ||
    ...
    for (const doc of unmatchedBillingSnap.docs) {
        ...
        governanceIssues.push({
            ...
            description: `${asTrimmedString(item.eventName) || 'evento'} | pagamento ${asTrimmedString(item.paymentId) || asTrimmedString(payment.id) || '-'} | status ${asTrimmedString(item.status) || 'unmatched'}`,
    ...
    for (const doc of pendingAccountantSnap.docs) {
        ...
        governanceIssues.push({
            ...
            description: `${asTrimmedString(companyData.nomeFantasia) || asTrimmedString(companyData.razaoSocial) || doc.id} | contador: ${asTrimmedString(pending.accountantEmail) || '-'}`,
            entityId: doc.id,
    ...
    for (const doc of officeBillingSnap.docs) {
        const item = asRecord(doc.data());
        if (item.officePartnershipWaiverActive === true)
            continue;
        governanceIssues.push({
            ...
            description: `${asTrimmedString(item.officeName) || doc.id} | ${asTrimmedString(item.email) || '-'} | status ${asTrimmedString(item.officeBillingStatus) || 'pending_setup'}`,
            entityId: doc.id,
            updatedAt: timestampToIsoString(item.updatedAt || item.createdAt),
        });
    }
    ...
    governanceIssues.sort((a, b) => asTrimmedString(b.updatedAt).localeCompare(asTrimmedString(a.updatedAt)));
    return {
        ok: true,
        leads,
        onboardings,
...
exports.publicCreateEmployeeTesterLead = functions.https.onCall(async (data) => {
    ...
    if (!fullName || !email) {
        throw new functions.https.HttpsError('invalid-argument', 'Informe nome e email para entrar na fila de testes.');
    }
...
exports.platformMarkEmployeeTesterPlayStoreIncluded = functions.https.onCall(async (data, context) => {
    ...
    const nextStatus = asTrimmedString(lead.status) === 'access_sent' ? 'access_sent' : 'play_store_ready';
    await leadRef.set({
        status: nextStatus,
        playStoreTesterIncludedAt: lead.playStoreTesterIncludedAt ?? admin.firestore.FieldValue.serverTimestamp(),
        updatedAt: admin.firestore.FieldValue.serverTimestamp(),
...
exports.platformReleaseEmployeeTesterAccess = functions.https.onCall(async (data, context) => {
    ...
    if (!email || !nome) {
        throw new functions.https.HttpsError('failed-precondition', 'Lead sem nome ou email valido para liberar o acesso.');
    }
    if (!lead.playStoreTesterIncludedAt && asTrimmedString(lead.status) !== 'access_sent') {
        throw new functions.https.HttpsError('failed-precondition', 'Marque primeiro que o email foi incluido manualmente na lista fechada da Play Store.');
    }
    ...
    await admin.firestore().collection('users').doc(userRecord.uid).set({
        ...
        endereco: [asTrimmedString(lead.city), asTrimmedString(lead.state)]
            .filter((item) => !!item)
            .join(' / ') || null,
        apelido: null,
        documento: null,
    ...
    try {
        ...
        throw new functions.https.HttpsError('internal', 'Tester incluido, mas falhou o envio do email de acesso.');
    }
    const beforeLead = {
        status: asTrimmedString(lead.status),
    ...
    await leadRef.set({
        ...
        playStoreTesterIncludedAt: lead.playStoreTesterIncludedAt ?? admin.firestore.FieldValue.serverTimestamp(),
        playStoreReleasedAt: lead.playStoreReleasedAt ?? admin.firestore.FieldValue.serverTimestamp(),
...
exports.platformReleaseEmployeeTesterRealAccess = functions.https.onCall(async (data, context) => {
    ...
    if (!testerUid || asTrimmedString(lead.status) !== 'access_sent') {
        throw new functions.https.HttpsError('failed-precondition', 'Libere primeiro o acesso de teste antes de liberar o ambiente real.');
    }
    ...
    try {
        ...
        throw new functions.https.HttpsError('internal', 'Ambiente real liberado, mas falhou o envio do email de migracao.');
    }
    await writeAudit({
        claims,
        module: 'platform',
...
function maxTimestampIsoFromDocs(docs, fields) {
    let latest = null;
...
exports.platformGetEmployeeTesterUsageSummary = functions.https.onCall(async (data, context) => {
    ...
    const [taskSnap, ordersSnap, punchSnap, justificationSnap, paymentsSnap, debtsSnap, movementsSnap, consentSnap,] = await Promise.all([
...
exports.publicTrackMarketingEvent = functions.https.onCall(async (data) => {
    ...
    if (!eventName || !visitorId || !sessionId) {
        throw new functions.https.HttpsError('invalid-argument', 'eventName, visitorId e sessionId sao obrigatorios.');
    }
    ...
    await admin.firestore().runTransaction(async (tx) => {
        ...
        tx.set(visitorRef, {
            ...
            firstSourceBucket: visitorSnap.exists ? visitorSnap.get('firstSourceBucket') ?? sourceBucket : sourceBucket,
            sourceBucket,
            utmSource,
            utmMedium,
            utmCampaign,
...
exports.accountantGetPartnerContact = functions.https.onCall(async (_data, context) => {
    ...
    const snap = await admin
        ...
        .get();
    if (snap.empty) {
        throw new functions.https.HttpsError('not-found', 'Cadastro da empresa suprema nao encontrado.');
    }
...
exports.publicCreateSalesPreRegistration = functions.https.onCall(async (data) => {
    ...
    const plan = publicSalesPlanEntries(publicConfig).find((item) => asTrimmedString(item.code).toLowerCase() == planCode) ?? {};
    ...
    const accountantName = implementationMode === 'accountant' ? (accountantNameRaw || customerName) : accountantNameRaw;
    const accountantEmail = implementationMode === 'accountant' ? (accountantEmailRaw || customerEmail) : accountantEmailRaw;
    const tracking = asRecord(data?.tracking);
    const visitorId = asTrimmedString(tracking.visitorId);
    const sessionId = asTrimmedString(tracking.sessionId);
    const utmSource = sanitizeMarketingKey(tracking.utmSource);
    ...
    if (!customerName || !customerEmail) {
        throw new functions.https.HttpsError('invalid-argument', 'Informe nome e email da empresa para continuar.');
    }
    if (implementationMode == 'accountant' &&
...
exports.platformIssueTrial90DayInvite = functions.https.onCall(async (data, context) => {
    ...
    const companyEmail = companyEmailRaw && companyEmailRaw.includes('@') ? companyEmailRaw : '';
    const token = generatePublicToken();
    const tokenHash = hashPublicToken(token);
    const now = Date.now();
    const expiresAt = admin.firestore.Timestamp.fromMillis(now + 30 * 24 * 60 * 60 * 1000);
    const inviteUrlCompany = `https://gestao-ponto-certo.com/cadastro-empresa?trialToken=${encodeURIComponent(token)}`;
    const inviteUrlAccountant = `https://gestao-ponto-certo.com/cadastro-empresa-contador?trialToken=${encodeURIComponent(token)}`;
    ...
    const cnpjFormattedForEmail = companyCnpjDigits.length === 14 ? formatBrazilCnpjForDisplay(companyCnpjDigits) : '';
    const emailCfg = obterConfigEmail();
    const missingEmailCfg = missingInviteConfig(emailCfg);
    if (missingEmailCfg.length) {
        throw new functions.https.HttpsError('failed-precondition', `Envio de convite indisponivel: configuracao de email incompleta (${missingEmailCfg.join(', ')}).`);
    }
    ...
    <div style="font-family: Arial, Helvetica, sans-serif; color: #1f2937; line-height: 1.55; max-width: 640px;">
      ...
      <p style="margin: 0 0 14px 0;">
        Meu nome é <strong>${escapeHtml(OFFICE_FOUNDER_DISPLAY_NAME)}</strong>, sou fundador do <strong>Ponto Certo</strong> e também empresário na área de obras e serviços elétricos.
      </p>
...
exports.platformTrial90DayFollowup = functions.pubsub
    ...
    for (const doc of expiredSnap.docs) {
        ...
        const recipients = Array.from(new Set([companyEmail, ownerEmail].filter((e) => e && e.includes('@'))));
        const html = `
        <div style="font-family: Arial, Helvetica, sans-serif; color: #111; line-height: 1.5;">
...
exports.companyStartPrepaidPlanFromTrial = functions.https.onCall(async (_data, context) => {
    ...
    if (!['trialing', 'trial_expired'].includes(billingStatus)) {
        throw new functions.https.HttpsError('failed-precondition', 'A adesao pre-paga so pode ser iniciada para empresas em trial ou com trial encerrado.');
    }
    ...
    if (plan.monthlyPriceCents <= 0) {
        throw new functions.https.HttpsError('failed-precondition', 'Nao foi possivel calcular o valor do plano para conversao do trial.');
    }
    ...
    const nextDueDate = admin.firestore.Timestamp.fromDate(new Date(`${provisioned.nextDueDate}T12:00:00.000Z`));
    const nextCommercial = {
        ...
        platformNote: 'Trial convertido para adesao pre-paga. Boleto inicial emitido com vencimento em 5 dias.',
        billingIntegration: {
            ...currentBilling,
            provider: 'asaas',
            accessManagedByGateway: true,
    ...
    if (ownerEmail) {
        const emailCfg = obterConfigEmail();
        const companyName = asTrimmedString(companyData.nomeFantasia) || asTrimmedString(companyData.razaoSocial);
        await enviarEmailHtml({
            toEmail: ownerEmail,
...
exports.publicRequestPasswordResetEmail = functions.https.onCall(async (data) => {
    ...
    const missingEmailCfg = missingInviteConfig(emailCfg);
    if (missingEmailCfg.length) {
        throw new functions.https.HttpsError('failed-precondition', `Envio de email indisponivel: configuracao incompleta (${missingEmailCfg.join(', ')}).`);
    }
    try {
        const resetLink = await admin.auth().generatePasswordResetLink(email);
...
exports.publicGetAccountingOfficeSignupPrefill = functions.https.onCall(async (data) => {
    ...
    if (!accSnap.empty) {
        ...
        const expiresAt = item.tokenExpiresAt;
        const expired = expiresAt instanceof admin.firestore.Timestamp
    ...
    const trialSnap = await trialInviteRef()
        ...
        .get();
    if (trialSnap.empty) {
        throw new functions.https.HttpsError('not-found', 'Convite de escritorio nao encontrado ou invalido.');
    }
    const t = asRecord(trialSnap.docs[0].data());
    const tExpires = t.tokenExpiresAt;
    const tExpired = tExpires instanceof admin.firestore.Timestamp
...
exports.publicSubmitAccountingOfficeSignup = functions.https.onCall(async (data) => {
    ...
    let trialInviteDocForOffice = null;
    let invitedByName = '';
    let invitedByEmail = '';
    if (token) {
        const tokenHash = hashPublicToken(token);
        ...
        if (!accSnap.empty) {
            ...
            const expired = expiresAt instanceof admin.firestore.Timestamp
                ...
                : false;
            if (expired) {
                throw new functions.https.HttpsError('failed-precondition', 'Convite de escritorio expirado.');
            }
            ...
            if (tStatus === 'used' || tStatus === 'deleted') {
                throw new functions.https.HttpsError('failed-precondition', 'Este convite de teste ja foi utilizado ou esta encerrado.');
            }
            ...
            const tExpired = tExpires instanceof admin.firestore.Timestamp
                ...
                : false;
            if (tExpired) {
                throw new functions.https.HttpsError('failed-precondition', 'Convite de teste de 30 dias expirado.');
            }
            ...
            if (expectedAccountant && email !== expectedAccountant) {
                throw new functions.https.HttpsError('failed-precondition', 'Para usar este link de teste, conclua o cadastro com o mesmo email do escritorio indicado no convite (email do contador).');
            }
...
exports.accountantRegisterCompanyIndication = functions.https.onCall(async (data, context) => {
    ...
    const officeBillingChoiceDefault = asTrimmedString(accountantProfile.officeBillingChoiceDefault).toLowerCase() === 'company'
        ? 'company'
        : 'office';
    ...
    if (!responsibleName || !legalName || !tradeName || cnpj.length !== 14) {
        throw new functions.https.HttpsError('invalid-argument', 'Preencha os dados principais da empresa.');
    }
    if (!phone || !companyEmail || !address) {
        throw new functions.https.HttpsError('invalid-argument', 'Telefone, email da empresa e endereco sao obrigatorios.');
    }
    ...
    if (companyAccessEnabled && !companyAccessEmail) {
        throw new functions.https.HttpsError('invalid-argument', 'Informe o email de acesso da empresa quando o acesso estiver habilitado.');
    }
    ...
    const commercialSettings = buildDefaultCommercialSettings({
        ...
        commercialSettings: {
            ...
            platformNote: `Empresa indicada por escritorio contabil. Mensalidade base de R$ 97,90 com pagador ${billingChoice === 'company' ? 'empresa' : 'escritorio'}.`,
        },
    });
    const settingsRef = admin.firestore().collection('company_settings').doc(companyId);
    await settingsRef.set({
    ...
    const companyDataForProvision = { ...companyData };
    try {
        const claimsForProvision = {
            uid,
            companyId,
            role: 'ACCOUNTANT',
            employeeId: uid,
...
exports.publicRegisterCompanyDirectSignup = functions.https.onCall(async (data) => {
    ...
    if (trialInviteToken) {
        ...
        if (inviteQuery.empty) {
            throw new functions.https.HttpsError('not-found', 'Convite de teste nao encontrado ou invalido.');
        }
        ...
        const expiresAtMillis = inviteExpiresAt instanceof admin.firestore.Timestamp
            ...
            : inviteExpiresAt instanceof Date
                ...
                : Number.NaN;
        if (status !== 'issued') {
            throw new functions.https.HttpsError('failed-precondition', 'Convite de teste ja utilizado ou indisponivel.');
        }
        if (!Number.isFinite(expiresAtMillis) || expiresAtMillis < Date.now()) {
            ...
            throw new functions.https.HttpsError('failed-precondition', 'Convite de teste expirado. Solicite um novo link.');
        }
        if (invitedCompanyEmail && invitedCompanyEmail !== ownerEmail && invitedCompanyEmail !== companyEmail) {
            throw new functions.https.HttpsError('permission-denied', 'Este convite de teste nao corresponde ao email informado.');
        }
    ...
    const publicConfigSnap = await publicSalesConfigRef().get();
    const publicConfig = buildDefaultPublicSalesConfig(asRecord(publicConfigSnap.data()));
    const selectedPlan = asRecord(classification.companyPlan === 'SOLO' ? publicConfig.planSolo : publicConfig.planEquipe);
...
exports.accountantRegisterCompanyTrial30d = functions.https.onCall(async (data, context) => {
    ...
    const graceUntil = admin.firestore.Timestamp.fromMillis(Date.now() + trialDays * 24 * 60 * 60 * 1000);
    const billingData = {
        provider: 'manual',
        accessManagedByGateway: true,
        customerId: '',
...
exports.publicSubmitSalesOnboardingRequest = functions.https.onCall(async (data) => {
    ...
    if (!legalName || !tradeName || !document || !ownerName || !ownerEmail || !phone) {
        throw new functions.https.HttpsError('invalid-argument', 'Preencha razao social, nome fantasia, documento, responsavel, email e telefone.');
    }
    if (onboardingMode !== 'accountant' &&
        ((accountantName && !accountantEmail) || (!accountantName && accountantEmail))) {
        throw new functions.https.HttpsError('invalid-argument', 'Informe nome e email do contador para vincular o escritorio a empresa.');
    }
    ...
    for (const item of uploadsRaw.slice(0, 6)) {
        ...
        if (buffer.byteLength > 5 * 1024 * 1024) {
            throw new functions.https.HttpsError('invalid-argument', 'Cada arquivo deve ter no maximo 5 MB.');
        }
    ...
    const missing = [];
    if (!taxRegime)
        missing.push('regime tributario');
    ...
    if (!state)
        missing.push('estado');
    if (!legalNature)
        missing.push('natureza juridica');
    if (!companySize)
        missing.push('porte da empresa');
    if (!mainCnae)
        missing.push('cnae principal');
    if (!mainCnaeDescription)
        missing.push('descricao do cnae principal');
    if (!municipalCode)
        missing.push('codigo do municipio / IBGE');
    if (businessCategory === 'service' && !standardServiceCode) {
        missing.push('codigo padrao de servico');
    }
    if (!certificatePassword)
        missing.push('senha do certificado digital');
    if (!uploadCategories.includes('cartao_cnpj'))
        missing.push('cartao CNPJ');
    ...
    if (!uploadCategories.includes('contrato_social'))
        missing.push('contrato social');
    if (!uploadCategories.includes('certificado_digital_a1')) {
        missing.push('certificado digital A1');
    }
    if (missing.length > 0) {
        throw new functions.https.HttpsError('invalid-argument', `Nao foi possivel concluir o onboarding sem a base completa para emissao e operacao. Falta: ${missing.join(', ')}.`);
    }
    await requestRef.set({
        status: 'submitted',
        formData: {
...
exports.platformGenerateImplementationCharge = functions.https.onCall(async (data, context) => {
    ...
    const implementationFeeCents = data?.implementationFeeCents == null
        ? Number(item.implementationFeeCents ?? 0) || 0
        : parseNonNegativeInt(data.implementationFeeCents, 'implementationFeeCents');
    if (implementationFeeCents <= 0) {
        throw new functions.https.HttpsError('failed-precondition', 'Onboarding sem valor de implantacao.');
    }
    ...
    if (charge == null) {
        throw new functions.https.HttpsError('failed-precondition', 'Onboarding sem customerId ou sem valor de implantacao.');
    }
    return {
        ok: true,
        requestId,
...
exports.platformFinalizeSalesOnboarding = functions.https.onCall(async (data, context) => {
    ...
    if (!['submitted', 'implementation_completed'].includes(status)) {
        throw new functions.https.HttpsError('failed-precondition', 'O onboarding precisa estar enviado para virar empresa operacional.');
    }
    ...
    if (finalized.companyId) {
        await admin
            ...
            .doc('current')
            .set({
            implementationCharge: implementationCharge == null
                ? admin.firestore.FieldValue.delete()
                : {
            ...
            implementationChargeAutomationError: implementationChargeError || admin.firestore.FieldValue.delete(),
            updatedAt: admin.firestore.FieldValue.serverTimestamp(),
        }, { merge: true });
    }
    return {
...
exports.platformUpdateCompanyCommercialSettings = functions.https.onCall(async (data, context) => {
    ...
    if (currentPlan === 'equipe' && requestedPlan === 'solo') {
        throw new functions.https.HttpsError('failed-precondition', 'Empresa ja enquadrada no plano Equipe nao pode voltar para Solo.');
    }
        ...
        if (!upgradePaid) {
            if (data?.acknowledgePlanUpgradeCharge !== true) {
                return {
                    ...
                    message: 'Migrar de Solo para Equipe exige cobranca de aquisicao. Confirme a geracao da cobranca antes de alterar o plano.',
                };
            }
            ...
            return {
                ...
                paymentLinkUrl: charge.paymentLinkUrl,
                amountCents: charge.amountCents,
                message: 'Cobranca de aquisicao gerada. O plano Equipe so pode ser aplicado apos o pagamento.',
            };
        }
    }
    const requestedBillingProvider = asTrimmedString(data?.billingProvider) ||
        asTrimmedString(currentBilling.provider) ||
        'manual';
    const requestedBillingAccessManaged = data?.billingAccessManagedByGateway == null
        ? currentBilling.accessManagedByGateway === true
        : data.billingAccessManagedByGateway === true;
    const requestedBillingGatewayStatus = asTrimmedString(data?.billingGatewayStatus) ||
        asTrimmedString(currentBilling.status) ||
        'pending_setup';
    const requestedCommercialBillingStatus = asTrimmedString(data?.billingStatus) || currentCommercial.billingStatus;
    const effectiveBillingStatus = requestedBillingAccessManaged && requestedBillingProvider !== 'manual'
        ? requestedBillingGatewayStatus
        : requestedCommercialBillingStatus;
    const afterCommercial = {
        ...currentCommercial,
        plan: requestedPlan,
        businessTier: requestedPlan === 'equipe' ? 'empresa' : 'mei',
        lifecycleStatus: asTrimmedString(data?.lifecycleStatus) || currentCommercial.lifecycleStatus,
        billingStatus: effectiveBillingStatus,
        approvalStatus: asTrimmedString(data?.approvalStatus) || currentCommercial.approvalStatus,
        allowLogin: data?.allowLogin == null
            ? currentCommercial.allowLogin
            : data.allowLogin === true,
        requiresApproval: data?.requiresApproval == null
            ? currentCommercial.requiresApproval
            : data.requiresApproval === true,
        seatsIncluded: data?.seatsIncluded == null
            ? currentCommercial.seatsIncluded
            : parseNonNegativeInt(data.seatsIncluded, 'seatsIncluded'),
        contractedAppUsers: data?.contractedAppUsers == null
            ? currentCommercial.contractedAppUsers
            : parseNonNegativeInt(data.contractedAppUsers, 'contractedAppUsers'),
        baseSystemPriceCents: data?.baseSystemPriceCents == null
            ? currentCommercial.baseSystemPriceCents
            : parseNonNegativeInt(data.baseSystemPriceCents, 'baseSystemPriceCents'),
        extraAppUserPriceCents: data?.extraAppUserPriceCents == null
        ...
        pricingModel: asTrimmedString(data?.pricingModel) || currentCommercial.pricingModel,
        billingIntegration: {
            ...currentBilling,
            provider: requestedBillingProvider,
            accessManagedByGateway: requestedBillingAccessManaged,
            billingType: asTrimmedString(data?.billingType) ||
                asTrimmedString(currentBilling.billingType) ||
                'BOLETO',
            cycle: asTrimmedString(data?.billingCycle) ||
                asTrimmedString(currentBilling.cycle) ||
                'MONTHLY',
            customerId: asTrimmedString(data?.billingCustomerId) ||
                asTrimmedString(currentBilling.customerId),
            subscriptionId: asTrimmedString(data?.billingSubscriptionId) ||
                asTrimmedString(currentBilling.subscriptionId),
            paymentLinkUrl: asTrimmedString(data?.billingPaymentLinkUrl) ||
                asTrimmedString(currentBilling.paymentLinkUrl),
            checkoutUrl: asTrimmedString(data?.billingCheckoutUrl) ||
                asTrimmedString(currentBilling.checkoutUrl),
            externalReference: asTrimmedString(data?.billingExternalReference) ||
            ...
            graceDays: data?.billingGraceDays == null
                ? (() => {
    ...
    const additionalAppUsers = Math.max(0, Number(afterCommercial.contractedAppUsers ?? 0) -
        Number(afterCommercial.seatsIncluded ?? 0));
    const calculatedMonthlyPriceCents = Number(afterCommercial.baseSystemPriceCents ?? 0) +
...
exports.platformProvisionCompanyBillingAsaas = functions.https.onCall(async (data, context) => {
    ...
    const nextDueDate = parseOptionalDate(data?.nextDueDate || toIsoDateString(new Date(Date.now() + 24 * 60 * 60 * 1000)), 'nextDueDate');
    if (nextDueDate == null) {
    ...
    const currentBilling = asRecord(currentCommercial.billingIntegration);
    const companyData = asRecord(settingsData.companyData);
    const externalReference = asTrimmedString(data?.externalReference) ||
        asTrimmedString(currentBilling.externalReference) ||
        companyId;
    ...
    if (!customerId) {
        ...
        customerId = asTrimmedString(customerResult.id);
        if (!customerId) {
            throw new functions.https.HttpsError('internal', 'Asaas nao retornou customerId ao criar cliente.');
        }
    }
    ...
    if (subscriptionId) {
        subscriptionResult = await asaasRequest(cfg, `/subscriptions/${encodeURIComponent(subscriptionId)}`, {
            method: 'PUT',
            body: {
                ...subscriptionPayload,
                updatePendingPayments: true,
    ...
    const providerStatus = asTrimmedString(subscriptionResult.status).toLowerCase() || 'pending_payment';
    const afterCommercial = {
        ...currentCommercial,
        billingStatus: providerStatus == 'active' || providerStatus == 'paid'
            ? 'paid'
            : providerStatus,
        billingIntegration: {
            ...currentBilling,
            ...
            currentPeriodEnd: parseTimestampLike(subscriptionResult.nextDueDate) ?? currentBilling.currentPeriodEnd ?? null,
            blockReason: '',
            webhookReady: true,
        },
        updatedByPlatformAt: admin.firestore.FieldValue.serverTimestamp(),
...
exports.platformIssueCompanyActivationCode = functions.https.onCall(async (data, context) => {
    ...
    if (expiresInDays <= 0 || expiresInDays > 365) {
        throw new functions.https.HttpsError('invalid-argument', 'expiresInDays deve ficar entre 1 e 365.');
    }
    const settingsRef = admin.firestore().collection('company_settings').doc(companyId);
    const settingsSnap = await settingsRef.get();
    ...
    const now = Date.now();
    const expiresAt = admin.firestore.Timestamp.fromMillis(now + expiresInDays * 24 * 60 * 60 * 1000);
...
exports.redeemCompanyActivationCode = functions.https.onCall(async (data, context) => {
    ...
    await admin.firestore().runTransaction(async (tx) => {
        ...
        if (status !== 'issued') {
            throw new functions.https.HttpsError('failed-precondition', 'Este codigo nao esta disponivel para uso.');
        }
        if (expiresAt > 0 && expiresAt < Date.now()) {
            throw new functions.https.HttpsError('failed-precondition', 'Este codigo expirou. Solicite um novo codigo a plataforma.');
        }
        tx.set(codeRef, {
            status: 'redeemed',
...
exports.companyGetBillingManagementSnapshot = functions.https.onCall(async (_data, context) => {
    ...
    if (asTrimmedString(billing.provider).toLowerCase() !== 'asaas' || !subscriptionId) {
        throw new functions.https.HttpsError('failed-precondition', 'A empresa ainda nao possui assinatura Asaas ativa para gestao.');
    }
    const paymentsResult = await asaasRequest(cfg, `/subscriptions/${encodeURIComponent(subscriptionId)}/payments`);
    ...
    const renewalUrl = primaryAsaasPaymentUrl(selectedPayment) ||
    ...
    const nextBilling = {
        ...
        lastPaymentStatus: asTrimmedString(selectedPayment.status).toLowerCase() ||
            asTrimmedString(billing.lastPaymentStatus),
        lastPaymentAt: parseTimestampLike(selectedPayment.clientPaymentDate) ??
            billing.lastPaymentAt ??
            null,
        currentPeriodEnd: parseTimestampLike(selectedPayment.dueDate) ??
...
exports.companyUpdateAdditionalAppAccess = functions.https.onCall(async (data, context) => {
    ...
    const requestedContracted = parseNonNegativeInt(data?.contractedAppUsers ?? currentContracted, 'contractedAppUsers');
    const contractedAppUsers = Math.max(seatsIncluded, requestedContracted);
    const additionalAppUsers = Math.max(0, contractedAppUsers - seatsIncluded);
    const monthlyPriceCents = (Number(currentCommercial.baseSystemPriceCents ?? 0) || 0) +
    ...
    if (asTrimmedString(currentBilling.provider).toLowerCase() === 'asaas' &&
        asTrimmedString(currentBilling.subscriptionId)) {
...
exports.companyCancelBillingSubscription = functions.https.onCall(async (data, context) => {
    ...
    assertRole(claims, ['OWNER']);
    if (isSupremePlatformCompany(claims.companyId)) {
        throw new functions.https.HttpsError('permission-denied', 'A empresa suprema da plataforma nao utiliza cancelamento de assinatura por este fluxo.');
    }
    const cfg = assertAsaasConfigured();
    const settingsRef = admin.firestore().collection('company_settings').doc(claims.companyId);
    const settingsSnap = await settingsRef.get();
    ...
    if (asTrimmedString(currentBilling.provider).toLowerCase() !== 'asaas' || !subscriptionId) {
        throw new functions.https.HttpsError('failed-precondition', 'A empresa nao possui assinatura Asaas para cancelamento.');
    }
...
exports.asaasWebhook = functions.https.onRequest(async (req, res) => {
    ...
    try {
        ...
        if (!companyId) {
            const payment = asaasEventPaymentData(payload);
            const publicOnboarding = eventName === 'PAYMENT_CREATED' ||
        ...
        await admin.firestore().runTransaction(async (tx) => {
            ...
            const paymentAt = parseTimestampLike(payment.clientPaymentDate ||
                payment.paymentDate ||
                payment.confirmedDate ||
                payload.dateCreated) ?? nowTs;
            const dueAt = parseTimestampLike(payment.dueDate || payment.originalDueDate) ?? null;
            const delinquencyStartedAt = statusResult.markPaid
                ? null
                : currentBilling.delinquencyStartedAt ?? nowTs;
            const graceUntil = statusResult.markPaid
                ? null
                : graceDays > 0
                    ? admin.firestore.Timestamp.fromMillis(nowTs.toMillis() + graceDays * 24 * 60 * 60 * 1000)
                    : null;
            const nextBilling = {
                ...currentBilling,
                provider: 'asaas',
                ...
                status: statusResult.markPaid
                    ...
                    : statusResult.status === 'overdue' || statusResult.status === 'delinquent'
                        ? statusResult.status
                        : 'pending_payment',
                customerId: asTrimmedString(payment.customer) || asTrimmedString(currentBilling.customerId),
                subscriptionId: asTrimmedString(payment.subscription) || asTrimmedString(currentBilling.subscriptionId),
                externalReference: asTrimmedString(payment.externalReference) ||
                    asTrimmedString(currentBilling.externalReference) ||
                    companyId,
                ...
                lastPaymentStatus: asTrimmedString(payment.status) ||
            ...
            const nextCommercial = {
                ...
                billingIntegration: nextBilling,
                planUpgrade: paymentExternalReference === `plan_upgrade_${companyId}_solo_to_equipe`
                    ? {
                        ...currentPlanUpgrade,
                        currentPlan: 'solo',
                        targetPlan: 'equipe',
                        status: statusResult.markPaid ? 'paid' : nextBilling.status,
                        paymentId: asTrimmedString(payment.id) || asTrimmedString(currentPlanUpgrade.paymentId),
                        paymentLinkUrl: asTrimmedString(payment.invoiceUrl) ||
                            asTrimmedString(payment.bankSlipUrl) ||
...
exports.createEmployeeAccess = functions.https.onCall(async (data, context) => {
    ...
    if (missingConfig.length > 0) {
        throw new functions.https.HttpsError('failed-precondition', `Configuracao de convite incompleta. Defina: ${missingConfig.join(', ')}.`);
    }
    ...
    const companyName = String(perfilSolicitante.companyName ?? companyData?.nomeFantasia ?? '').trim();
    let userRecord;
    try {
        const senhaTemporaria = gerarSenhaTemporaria();
        userRecord = await admin.auth().createUser({
    ...
    try {
        ...
        throw new functions.https.HttpsError('internal', 'Acesso criado, mas falhou o envio do email de acesso. Tente novamente.');
    }
    await writeAudit({
        claims,
        module: 'employees',
...
exports.runtimeIncidentExecuteSafeAction = functions.https.onCall(async (data, context) => {
    ...
    if (!analysis.autoFixEligible) {
        throw new functions.https.HttpsError('failed-precondition', 'Este incidente ainda nao possui uma acao segura automatizavel.');
    }
    ...
    if (analysis.recommendedActionType !== 'refresh_fiscal_provisioning') {
        throw new functions.https.HttpsError('failed-precondition', 'A acao sugerida para este incidente nao possui executor automatico implementado.');
    }
    ...
    try {
        ...
        await incidentRef.set({
            ...
            status: 'resolved',
            resolutionNote: 'Correcao segura executada: reprocessamento da automacao fiscal da empresa.',
        ...
        const message = errorMessage(error, 'Falha ao executar a correcao segura deste incidente.');
        await incidentRef.set({
            assistantSummary: analysis.summary,
            recommendedAction: analysis.recommendedAction,
            recommendedActionType: analysis.recommendedActionType,
...
exports.runtimeIssueUpsertFromIncident = functions.https.onCall(async (data, context) => {
    ...
    await issueRef.set({
        companyId: claims.companyId,
        fingerprint,
        title: asTrimmedString(before.title) ||
            `${asTrimmedString(incidentData.category) || 'runtime'} | ${asTrimmedString(incidentData.source) || 'app'}`,
        description: asTrimmedString(before.description) || asTrimmedString(incidentData.message),
        ...
        recommendedAction: asTrimmedString(before.recommendedAction) || analysis.recommendedAction,
        recommendedActionType: asTrimmedString(before.recommendedActionType) || analysis.recommendedActionType,
        assistantSummary: asTrimmedString(before.assistantSummary) || analysis.summary,
...
exports.assistantSendMessage = HEAVY_RUNTIME.https.onCall(async (data, context) => {
    ...
    if (missingCfg.length > 0) {
        throw new functions.https.HttpsError('failed-precondition', `Assistente nao configurado. Defina: ${missingCfg.join(', ')}.`);
    }
    ...
    const currentRequestCount = asTrimmedString(usageData.periodKey) === periodKey
        ? Number(usageData.requestCount ?? 0)
        : 0;
    const currentTokenCount = asTrimmedString(usageData.periodKey) === periodKey
    ...
    const threadTitle = assistantThreadTitleFromMessage(asTrimmedString(threadData.title) || message);
    ...
    batch.set(admin.firestore().collection('company_settings').doc(claims.companyId), {
        companyId: claims.companyId,
        assistantUsage: {
            periodKey,
...
exports.assistantDebugLatestReplyEphemeral = functions.https.onRequest(async (req, res) => {
    ...
    for (const threadDoc of orderedThreads.slice(0, 10)) {
        ...
        for (const messageDoc of messagesSnap.docs) {
            ...
            if (asTrimmedString(data.authorType) !== 'assistant')
                continue;
...
exports.observabilityExportSupremeEphemeral = functions.https.onRequest(async (req, res) => {
    ...
    if (format === 'md' || format === 'markdown') {
        ...
        res.setHeader('Content-Disposition', `attachment; filename=\"observabilidade_${companyId}.md\"`);
        res.status(200).send(lines.join('\n'));
        return;
    }
    res.setHeader('Content-Type', 'application/json; charset=utf-8');
    res.setHeader('Content-Disposition', `attachment; filename=\"observabilidade_${companyId}.json\"`);
...
exports.syncCompanyProfile = functions.https.onCall(async (data, context) => {
    ...
    if (!companyName || !companyData) {
        throw new functions.https.HttpsError('invalid-argument', 'companyName e companyData sao obrigatorios.');
    }
...
exports.fiscalRefreshCompanyProvisioning = functions.https.onCall(async (_data, context) => {
    ...
    const companyName = asTrimmedString(userData.companyName || companyData.nomeFantasia || companyData.razaoSocial);
    const { recommended, focusProvisioning } = await refreshCompanyProvisioningState({
        claims,
        companyData,
    });
...
exports.setEmployeeActiveStatus = functions.https.onCall(async (data, context) => {
    ...
    await admin.firestore().runTransaction(async (tx) => {
        ...
        if (isSupremePlatformCompany(companyIdBefore) && !ativo) {
            throw new functions.https.HttpsError('permission-denied', 'Contas da empresa suprema da plataforma nao podem ser desativadas.');
        }
        tx.set(employeeRef, {
            companyId: claims.companyId,
            ativo,
...
exports.employeesGetBaseSnapshot = functions.https.onCall(async (_data, context) => {
    ...
    const all = snapshot.docs
        ...
        const updatedAtIso = updatedAtRaw instanceof admin.firestore.Timestamp
...
exports.paymentsCreate = functions.https.onCall(async (data, context) => {
    ...
    const paymentType = normalizePaymentType(data?.paymentType, employeeProfile.compensationType);
    ...
    if (paymentType === 'MONTHLY' && activeExistingPayments.length > 0) {
        ...
        throw new functions.https.HttpsError('already-exists', `Pagamento mensal ja existe para esta competencia. Status: ${existingStatus}. Referencia: ${paymentLaunchReferenceLabel(latestExisting)}.`);
    }
    ...
    await admin.firestore().runTransaction(async (tx) => {
        ...
        tx.set(ref, after);
        await applyRuntimeSummaryDelta(tx, claims.companyId, 'finance', null, 0, paymentSummaryBucket(after), paymentSummaryAmount(after), runtimeSummaryCurrent);
        await writeAudit({
            claims,
            module: 'payments',
            action: 'create',
...
exports.paymentsCreateBulk = functions.https.onCall(async (data, context) => {
    ...
    if (competenceMonth < 1 || competenceMonth > 12 || rawItems.length === 0) {
        throw new functions.https.HttpsError('invalid-argument', 'competencia ou lote de pagamentos invalido.');
    }
    ...
    if (items.length === 0) {
        throw new functions.https.HttpsError('invalid-argument', 'Nenhum funcionario valido informado para o lote.');
    }
    const created = [];
    const skipped = [];
    const failed = [];
    ...
    for (const item of items) {
        ...
        try {
            ...
            item.paymentType = normalizePaymentType(item.paymentType, employeeProfile.compensationType);
            const existing = await admin
                .firestore()
                .collection('payments')
                .where('companyId', '==', claims.companyId)
            ...
            if (item.paymentType === 'MONTHLY' && activeExistingPayments.length > 0) {
                ...
                skipped.push({
                    employeeId: item.employeeId,
                    message: `Pagamento mensal ja existe para esta competencia. ` +
...
exports.paymentsUpdate = functions.https.onCall(async (data, context) => {
    ...
    if (!paymentId || !employeeId || competenceMonth < 1 || competenceMonth > 12) {
        throw new functions.https.HttpsError('invalid-argument', 'paymentId/employeeId/competencia invalidos.');
    }
    ...
    await admin.firestore().runTransaction(async (tx) => {
        ...
        if (currentStatus === 'CANCELED') {
            throw new functions.https.HttpsError('failed-precondition', 'Pagamento cancelado nao pode ser editado.');
        }
        const after = {
            employeeId,
            competenceYear,
        ...
        await applyRuntimeSummaryDelta(tx, claims.companyId, 'finance', paymentSummaryBucket(asRecord(before)), paymentSummaryAmount(asRecord(before)), paymentSummaryBucket({ ...asRecord(before), ...after }), paymentSummaryAmount({ ...asRecord(before), ...after }), runtimeSummaryCurrent);
        await writeAudit({
            claims,
            module: 'payments',
            action: 'update',
...
exports.debtsCreate = functions.https.onCall(async (data, context) => {
    ...
    await admin.firestore().runTransaction(async (tx) => {
        ...
        tx.set(ref, after);
        await applyRuntimeSummaryDelta(tx, claims.companyId, 'finance', null, 0, debtSummaryBucket(after), debtSummaryAmount(after), runtimeSummaryCurrent);
        await writeAudit({
            claims,
            module: 'debts',
            action: 'create',
...
exports.debtsSettle = functions.https.onCall(async (data, context) => {
    ...
    await admin.firestore().runTransaction(async (tx) => {
        ...
        tx.set(ref, after, { merge: true });
        await applyRuntimeSummaryDelta(tx, claims.companyId, 'finance', debtSummaryBucket(asRecord(before)), debtSummaryAmount(asRecord(before)), debtSummaryBucket({ ...asRecord(before), ...after }), debtSummaryAmount({ ...asRecord(before), ...after }), runtimeSummaryCurrent);
        await writeAudit({
            claims,
            module: 'debts',
            action: 'settle',
...
exports.fiscalSyncFocusCompany = HEAVY_RUNTIME.https.onCall(async (_data, context) => {
    ...
    const settingsData = await mergeFiscalSecureSettings(claims.companyId, asRecord(settingsSnap.data()));
    ...
    const setup = asRecord(settingsData.fiscalRealIntegration);
    if (!providerIsFocus(asTrimmedString(setup.provider))) {
        throw new functions.https.HttpsError('failed-precondition', 'Selecione Focus NFe como provedor para usar esta sincronizacao.');
    }
    const result = await syncFocusCompany({
        claims,
        companyData,
...
exports.fiscalIssueServiceInvoice = HEAVY_RUNTIME.https.onCall(async (data, context) => {
    ...
    functions.logger.info('fiscalIssueServiceInvoice start', {
        invoiceId,
        companyId: claims.companyId,
        invoiceCompanyId: String(invoiceData.companyId ?? ''),
    ...
    if (!providerIsFocus(asTrimmedString(setup.provider)) &&
        ...
        throw new functions.https.HttpsError('failed-precondition', 'Configure a Base URL da emissao real antes de emitir.');
    }
    ...
    const officialNumber = extractFocusOfficialNfseNumber(pr) ||
    ...
    const officialPortalUrl = asTrimmedString(providerResponse.officialPortalUrl) ||
        asTrimmedString(providerResponse.portalUrl) ||
        asTrimmedString(providerResponse.url);
    const protocol = asTrimmedString(providerResponse.protocol) ||
    ...
    const status = normalizeOfficialInvoiceStatus(statusRaw, providerResponse.provider, 'APPROVED');
    functions.logger.info('fiscalIssueServiceInvoice provider result', {
        ...
        rawStatus: String(providerResponse.status ?? ''),
...
exports.fiscalCancelServiceInvoice = HEAVY_RUNTIME.https.onCall(async (data, context) => {
    ...
    if (!providerIsFocus(asTrimmedString(setup.provider)) &&
        !asTrimmedString(setup.apiBaseUrl)) {
        throw new functions.https.HttpsError('failed-precondition', 'Configure a Base URL da emissao real antes de cancelar.');
    }
    let providerResponse;
    try {
        providerResponse = await callFiscalProvider({
    ...
    const status = normalizeOfficialInvoiceStatus(providerResponse.status, providerResponse.provider, 'CANCELED');
    const cancelProtocol = asTrimmedString(providerResponse.cancelProtocol) ||
...
exports.fiscalRefreshServiceInvoiceStatus = HEAVY_RUNTIME.https.onCall(async (data, context) => {
    ...
    if (!providerIsFocus(asTrimmedString(setup.provider)) &&
        !asTrimmedString(setup.apiBaseUrl)) {
        throw new functions.https.HttpsError('failed-precondition', 'Configure a integracao fiscal real antes de consultar o status oficial.');
    }
    let providerResponse;
    try {
        providerResponse = await callFiscalProvider({
    ...
    functions.logger.info('fiscalRefreshServiceInvoiceStatus provider result', {
        ...
        environment: asTrimmedString(providerResponse.environment),
        rawStatus: String(providerResponse.status ?? ''),
        officialNumber: asTrimmedString(providerResponse.officialNumber) ||
            asTrimmedString(providerResponse.invoiceNumber) ||
            asTrimmedString(providerResponse.number) ||
            asTrimmedString(providerResponse.nfseNumber),
    });
    ...
    if (persisted.status.toUpperCase() === 'APPROVED' ||
...
exports.fiscalReconcileProcessingInvoices = HEAVY_RUNTIME.https.onCall(async (data, context) => {
    ...
    if (invoiceIds.length === 0) {
        throw new functions.https.HttpsError('invalid-argument', 'invoiceIds obrigatorio para reconciliar notas em processamento.');
    }
    const settingsRef = admin.firestore().collection('company_settings').doc(claims.companyId);
    const settingsSnap = await settingsRef.get();
    const settingsData = await mergeFiscalSecureSettings(claims.companyId, asRecord(settingsSnap.data()));
    ...
    if (!providerIsFocus(asTrimmedString(setup.provider)) &&
        !asTrimmedString(setup.apiBaseUrl)) {
        throw new functions.https.HttpsError('failed-precondition', 'Configure a integracao fiscal real antes de reconciliar notas em processamento.');
    }
    const results = [];
    let updatedCount = 0;
    let failedCount = 0;
...
exports.fiscalDiagnoseServiceCatalog = HEAVY_RUNTIME.https.onCall(async (data, context) => {
    const claims = assertClaims(context);
    assertRole(claims, ['OWNER', 'MANAGER', 'ACCOUNTANT']);
    const applyFix = Boolean(data?.applyFix);
    ...
    const usesFocusNational = providerIsFocus(asTrimmedString(integrationSetup.provider)) &&
        asTrimmedString(integrationSetup.focusNfseApi).toLowerCase() === 'national';
    const onlyDigitsLocal = (value) => String(value ?? '')
        .replace(/\D/g, '')
        .trim();
    const normalizeCnaeLocal = (value) => {
        const digits = onlyDigitsLocal(value);
        if (!digits)
            return '';
        if (digits.length === 7)
            return digits;
        if (digits.length === 8 && digits.endsWith('0'))
            return digits.substring(0, 7);
        if (digits.length > 7)
            return digits.substring(0, 7);
        return digits;
    };
    const formatNationalSubitem = (digitsInput) => {
        const digits = onlyDigitsLocal(digitsInput);
        if (!digits)
            return '';
        const padded = digits.length === 5 ? `0${digits}` : digits.length === 4 ? `00${digits}` : digits;
        if (padded.length < 6)
            return digitsInput.trim();
    ...
    const suggestNationalCode = (params) => {
        ...
        const mentionsRepair = name.includes('repar') ||
        ...
        const mentionsBuilding = name.includes('edif') ||
            name.includes('predi') ||
            name.includes('imovel') ||
            name.includes('obra') ||
            name.includes('reforma');
        const mentionsRoads = name.includes('estrad') || name.includes('ponte') || name.includes('porto');
        ...
        if (cnae === '4321500') {
            ...
            if (is705Family || familyDigits.length < 6) {
                ...
                if (mentionsMaint || mentionsRepair) {
                    ...
                    const isPredial = name.includes('predial') ||
    ...
    for (const doc of snap.docs) {
        ...
        const serviceCodeRaw = String(raw.serviceCode ?? '').trim();
        const municipalCodeRaw = String(raw.municipalServiceCode ?? '').trim();
        const cnae = normalizeCnaeLocal(raw.cnae);
        ...
        if (usesFocusNational) {
            if (suggestion.code) {
                ...
                if (serviceDigits.length >= 6)
                    patch.serviceCode = formatNationalSubitem(serviceDigits);
                if (municipalDigits.length >= 6) {
                    patch.municipalServiceCode = formatNationalSubitem(municipalDigits);
                }
            }
        }
        if (cnae && cnae !== String(raw.cnae ?? '').trim()) {
            patch.cnae = cnae;
        }
        if (needsReview) {
            if (!String(raw.name ?? '').includes('[REVISAR CODIGO]')) {
...
exports.lookupBrazilCep = functions.https.onCall(async (data, context) => {
    ...
    if (!response.ok) {
        throw new functions.https.HttpsError('not-found', 'Nao foi possivel localizar dados para este CEP.');
    }
...
function buildEngineeringAgentMarkerBlock() {
    return [
...
function buildEngineeringAgentSystemPromptPontocerto() {
    return [
...
function detectEngineeringStackSnippet(snippet) {
...
function parseEngineeringAgentMarkers(reply) {
    const sliceBetween = (text, startTag, endTags) => {
...
async function engineeringAgentChatCompletion(params) {
    ...
    if (!response.ok) {
        ...
        throw new functions.https.HttpsError('internal', asTrimmedString(err.message) || 'Falha ao consultar o modelo.');
    }
    const choices = Array.isArray(payload.choices) ? payload.choices : [];
...
async function assertEngineeringAgentCaller(context) {
    const claims = assertClaims(context);
    assertSupremePlatformAccess(claims);
...
exports.engineeringAgentSendMessage = HEAVY_RUNTIME.https.onCall(async (data, context) => {
    ...
    const missingCfg = missingAssistantConfig(assistantCfg);
    if (missingCfg.length > 0) {
        throw new functions.https.HttpsError('failed-precondition', `Modelo indisponivel. Configure: ${missingCfg.join(', ')}.`);
    }
    const db = admin.firestore();
    const now = admin.firestore.FieldValue.serverTimestamp();
    let sessionRef = sessionId ? db.collection('engineering_agent_sessions').doc(sessionId) : null;
    let externalProject = null;
    let projectContextDoc = null;
    ...
    if (sessionRef) {
        ...
        const existingPid = asTrimmedString(asRecord(snap.data()).projectId) || ENGINEERING_AGENT_BUILTIN_PROJECT_ID;
        if (existingPid !== effectiveProjectId) {
            throw new functions.https.HttpsError('failed-precondition', 'Esta sessao pertence a outro projeto. Abra uma nova conversa.');
        }
    }
    else {
    ...
    if (effectiveProjectId !== ENGINEERING_AGENT_BUILTIN_PROJECT_ID && externalProject) {
        const snippet = asTrimmedString(externalProject.manifestSnippet) ||
        ...
        systemPrompt = buildEngineeringAgentSystemPromptExternal({
            ...
            stackDetected: asTrimmedString(externalProject.stackDetected) ||
...
exports.engineeringAgentGenerateCommand = HEAVY_RUNTIME.https.onCall(async (data, context) => {
    ...
    const sys = 'Output apenas uma linha PowerShell para Windows, sem markdown, sem aspas envolvendo a linha inteira, sem quebras de linha. ' +
...
function engineeringWorkerCommandNeedsApproval(commandLine) {
...
exports.engineeringAgentWorkerEnqueueJob = functions.https.onCall(async (data, context) => {
    ...
    const projectId = projectIdRaw.length > 0 ? projectIdRaw : ENGINEERING_AGENT_BUILTIN_PROJECT_ID;
    ...
    if (!commandLine) {
        throw new functions.https.HttpsError('invalid-argument', 'commandLine obrigatorio ou sessionId com lastCommand preenchido.');
    }
    let projectRoot = '';
    if (projectId === ENGINEERING_AGENT_BUILTIN_PROJECT_ID) {
        projectRoot = asTrimmedString(data?.workspaceRoot).slice(0, 2400);
        if (!projectRoot) {
            throw new functions.https.HttpsError('invalid-argument', 'Para o modo Ponto Certo informe workspaceRoot (pasta do clone local).');
        }
    }
    else {
...
exports.syncFinanceMovementRuntimeSummary = functions.firestore
    ...
    const companyId = asTrimmedString(afterData.companyId) || asTrimmedString(beforeData.companyId);
    if (!companyId)
        return;
    await admin.firestore().runTransaction(async (tx) => {
...
(0, vendas_public_flow_1.initVendasPublicExports)(exports, {
    obterConfigEmail,
    enviarEmailHtml,
    escapeHtml,
});
