...
exports.initVendasPublicExports = initVendasPublicExports;
/**
...
function syntheticCnpj(prefixDigit) {
...
async function logVendasPublic(action, details) {
...
async function enviarHtmlVendas(deps, params) {
...
async function assertTokenConviteValid(token) {
    ...
    const expMs = expira instanceof admin.firestore.Timestamp ? expira.toMillis() : 0;
    if (usado) {
        throw new functions.https.HttpsError('failed-precondition', 'Este convite ja foi utilizado.');
    }
    if (!expMs || expMs < Date.now()) {
        throw new functions.https.HttpsError('failed-precondition', 'Convite expirado. Solicite um novo link ao cliente.');
    }
...
function initVendasPublicExports(exportsObj, deps) {
    exportsObj.leadContabilidadeHttp = functions.https.onRequest(async (req, res) => {
        ...
        <div style="font-family: Arial, Helvetica, sans-serif; color: #111; line-height: 1.5;">
          ...
          <p style="color:#64748b;font-size:13px;">Se o link nao abrir, copie e cole no navegador:<br/>${deps.escapeHtml(conviteUrl)}</p>
        </div>`;
            try {
    ...
    exportsObj.vendasPublicSubmitContadorConvite = functions.https.onCall(async (data) => {
        ...
        if (!token || !nomeContador || !email || !senha) {
            throw new functions.https.HttpsError('invalid-argument', 'Preencha nome, email e senha.');
        }
        if (senha !== confirma) {
            throw new functions.https.HttpsError('invalid-argument', 'A confirmacao de senha nao confere.');
        }
        if (senha.length < 6) {
            throw new functions.https.HttpsError('invalid-argument', 'Senha muito curta (minimo 6 caracteres).');
        }
        const { snap: tokenSnap, leadId, emailContador } = await assertTokenConviteValid(token);
        if (email !== emailContador) {
            throw new functions.https.HttpsError('failed-precondition', 'Use o mesmo email do convite (email do contador indicado pelo cliente).');
        }
        const leadSnap = await leadsContabilidadeRef().doc(leadId).get();
        ...
        try {
            ...
            if (code === 'auth/email-already-exists') {
                throw new functions.https.HttpsError('already-exists', 'Este email ja possui conta. Entre com login ou use recuperar senha.');
            }
        ...
        const batch = admin.firestore().batch();
        batch.set(tokenSnap.ref, {
            usado: true,
            usado_em: ts,
            usado_por_uid: uid,
            updated_at: ts,
        }, { merge: true });
        batch.set(leadSnap.ref, {
            status: 'ativado',
...
//# sourceMappingURL=vendas_public_flow.js.map