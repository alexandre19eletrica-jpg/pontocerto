...
type AssistantRuntimeConfig = AssistantConfig & {
  /** Todas as empresas usam a mesma credencial: env Functions ou assistant_secure da suprema. */
  source: 'platform' | 'missing';
  keyPreview: string;
...
function primarySupremePlatformCompanyId(): string | null {
 ...
 */
async function resolveUnifiedAssistantCredential(): Promise<{
  apiKey: string;
  model: string;
  updatedByName?: string;
  updatedAtIso?: string;
...
async function obterConfigAssistantRuntime(_companyId: string): Promise<AssistantRuntimeConfig> {
  void _companyId;
  const platformCfg = obterConfigAssistant();
...
function missingAssistantConfig(cfg: AssistantConfig): string[] {
  ...
  if (!cfg.apiKey) {
    missing.push(
      'credencial central do assistente (variavel OPENAI_API_KEY nas Functions ou cadastro na empresa suprema)',
    );
  }
  return missing;
}
...
function assertSupremePlatformOwner(claims: Claims): void {
  assertRole(claims, ['OWNER']);
  if (!isSupremePlatformCompany(claims.companyId)) {
    throw new functions.https.HttpsError(
      'permission-denied',
      'Apenas o owner da empresa suprema pode executar esta operacao.',
    );
  }
}
...
const PUBLIC_OPEN_DEMO_CLOUD_FUNCTION_RUNTIME_SA_EMAIL =
  'firebase-adminsdk-fbsvc@pontocerto-e1dab.iam.gserviceaccount.com';
...
async function ensurePublicDemoUser(params: {
  ...
  if (
    params.uid !== PUBLIC_DEMO_OWNER_UID &&
    params.uid !== PUBLIC_DEMO_ACCOUNTANT_UID
  ) {
    throw new functions.https.HttpsError(
      'failed-precondition',
      'Perfil demo: UID fora do par canonico publico.',
    );
  }
      ...
      try {
        ...
        if (typed.code === 'auth/email-already-exists') {
          ...
          if (holder.uid !== params.uid) {
            throw new functions.https.HttpsError(
              'failed-precondition',
              'Demo indisponivel: o email do perfil demo esta associado a outra conta Firebase Auth. ' +
...
async function ensurePublicDemoWorkspace(companyId: string): Promise<void> {
  if (asTrimmedString(companyId) !== PUBLIC_DEMO_COMPANY_ID) {
    throw new functions.https.HttpsError(
      'failed-precondition',
      'ensurePublicDemoWorkspace: apenas o workspace demo publico e permitido.',
    );
  }
  await admin
    .firestore()
...
function analyzeRuntimeIncidentHeuristics(
  ...
  if (
    ...
    return {
      summary:
        'Falha classificada como indisponibilidade ou configuracao do assistente (credencial central da plataforma ou politicas/limites).',
      recommendedAction:
        'Equipe suprema: conferir OPENAI_API_KEY nas Functions ou credencial gravada na empresa suprema e revisar limites do assistente (company_settings). Clientes finais: suporte.',
...
function isDemoPlaceholderCompanyData(data: Record<string, unknown>): boolean {
  ...
  const endereco = String(data.endereco ?? '')
    .trim()
    .toLowerCase();
  if (endereco.includes('demonstrativo')) return true;
  return false;
}
...
async function enviarEmailHtml(params: {
  ...
  subject: string;
  html: string;
  /** Plain-text fallback (útil para clientes que preferem `text/plain`). */
  text?: string;
  fromEmail: string;
  sendgridKey: string;
  smtpUser: string;
  ...
  if (params.sendgridKey) {
    ...
    await sendgridMail.send(mailPayload);
    return;
  }
...
function compactSignupLeadOrigin(data: unknown): Record<string, string> | undefined {
...
async function provisionLightweightCompanyAccess(params: {
  ...
  password?: string;
  source: string;
  leadOrigin?: Record<string, string>;
}): Promise<{
  companyId: string;
  companyName: string;
  emailDispatched: boolean;
...
exports.platformListLightweightTestOffices = functions.https.onCall(async (_data, context) => {
  ...
  items.sort((a, b) =>
    String(b['officeId'] ?? '').localeCompare(String(a['officeId'] ?? '')),
  );
 ...
 */
async function evaluateStandaloneLightweightTestDeletionGate(
  companyId: string,
  ...
  if (!companyId || companyId === PUBLIC_DEMO_COMPANY_ID) {
    return {
      allowed: false,
      blockedReason:
        companyId === PUBLIC_DEMO_COMPANY_ID ? 'Workspace demo protegido.' : 'Empresa invalida.',
    };
  }
  ...
  if (settingsSnap.exists) {
    ...
    if (ds.lightweightProfilePending !== true) {
      return {
        allowed: false,
        blockedReason:
          'Somente registros marcados cadastro leve em company_settings (directSignup.lightweightProfilePending).',
      };
    }
  }
...
exports.platformDeleteLightweightTestCompany = functions.https.onCall(async (data, context) => {
  ...
  if (!companyId || companyId === PUBLIC_DEMO_COMPANY_ID) {
    throw new functions.https.HttpsError(
      'invalid-argument',
      'companyId obrigatorio (e nao pode ser o workspace demo).',
    );
  }
  ...
  if (!gate.allowed) {
    throw new functions.https.HttpsError(
      'failed-precondition',
      gate.blockedReason || 'Exclusao nao permitida.',
    );
  }
...
exports.platformDeleteLightweightTestOffice = functions.https.onCall(async (data, context) => {
  ...
  if (!officeId || officeId === PUBLIC_DEMO_OFFICE_ID) {
    throw new functions.https.HttpsError(
      'invalid-argument',
      'officeId obrigatorio (e nao pode ser o escritorio demo).',
    );
  }
  ...
  if (o.lightweightProfilePending !== true) {
    throw new functions.https.HttpsError(
      'failed-precondition',
      'So e permitido apagar escritorios ainda em cadastro leve (lightweightProfilePending).',
    );
  }
  ...
  if (linked.length > 0) {
    throw new functions.https.HttpsError(
      'failed-precondition',
      'Escritorio ainda tem empresas na carteira (company_settings). Remova ou apague essas empresas de teste primeiro.',
    );
  }
  ...
  if (storedCount > 0) {
    throw new functions.https.HttpsError(
      'failed-precondition',
      'Escritorio indica empresas vinculadas (linkedCompaniesCount). Confirme a carteira antes de apagar.',
    );
  }
  ...
  for (const d of accountantSnap.docs) {
    ...
    if (roleParaFirestore(u.role) !== 'ACCOUNTANT') {
      throw new functions.https.HttpsError(
        'failed-precondition',
        'Existem utilizadores nao-contador ligados a este officeId. Exclusao bloqueada.',
      );
    }
    if (u.lightweightProfilePending !== true) {
      throw new functions.https.HttpsError(
        'failed-precondition',
        'Contador ja nao esta em cadastro leve; exclusao abortada.',
      );
    }
  }
...
exports.platformSupremeSetCompanyAllowLogin = functions.https.onCall(async (data, context) => {
  ...
  if (!companyId || companyId === PUBLIC_DEMO_COMPANY_ID || isSupremePlatformCompany(companyId)) {
    throw new functions.https.HttpsError(
      'invalid-argument',
      'companyId invalido ou protegido (suprema/demo).',
    );
  }
  if (typeof data?.allowLogin !== 'boolean') {
    throw new functions.https.HttpsError(
      'invalid-argument',
      'Informe allowLogin (boolean).',
    );
  }
  ...
  const settingsData = asRecord(settingsSnap.data());
  const rawFlat: Record<string, unknown> = {...commercialSettings(settingsData)};
...
exports.platformSupremeDeleteOffice = functions.https.onCall(async (data, context) => {
  ...
  if (linked.length > 0) {
    throw new functions.https.HttpsError(
      'failed-precondition',
      `Escritorio ainda tem ${linked.length} empresa(s) na carteira. Desvincule ou apague antes.`,
    );
  }
...
function normalizeAudienceEmail(raw: string): string {
  return asTrimmedString(raw).toLowerCase();
}

function isPlausibleAudienceEmail(email: string): boolean {
...
function governanceAudienceAdd(
...
async function collectGovernanceAudienceEmailsInternal(): Promise<
  Array<{email: string; sources: string[]}>
> {
  ...
  const canonRows: CanonRow[] = await Promise.all(
    usersSnap.docs.map(async (doc) => {
      const data = asRecord(doc.data());
      const companyId = asTrimmedString(data.companyId);
      let accountantEmail = '';
  ...
  const bestMillis = new Map<string, number>();
  const canonicalByCompany = new Map<string, {ownerEmail: string; accountantEmail: string}>();
  ...
  const lightBuckets = new Map<string, FirebaseFirestore.QueryDocumentSnapshot>();
...
exports.platformGovernanceSendAudienceEmail = HEAVY_RUNTIME.https.onCall(async (data, context) => {
  ...
  const rawRecipients: unknown[] = Array.isArray(data?.recipients) ? (data.recipients as unknown[]) : [];

  if (!subject || subject.length > 180) {
    throw new functions.https.HttpsError(
      'invalid-argument',
      'Assunto obrigatorio (maximo 180 caracteres).',
    );
  }
  ...
  if (!trimmedBody || trimmedBody.length > 40000) {
    throw new functions.https.HttpsError(
      'invalid-argument',
      'Corpo obrigatorio (maximo 40000 caracteres).',
    );
  }
  ...
  if (!uniqueRecipients.length) {
    throw new functions.https.HttpsError(
      'invalid-argument',
      'Lista de destinatarios invalida ou vazia.',
    );
  }
  ...
  if (missingMail.length) {
    throw new functions.https.HttpsError(
      'failed-precondition',
      `Configure envio de email (${missingMail.join(', ')}).`,
    );
  }

  const html = `<div style="font-family: Arial, Helvetica, sans-serif; font-size: 14px; line-height: 1.55; color: #111; white-space: pre-wrap;">${escapeHtml(
    trimmedBody,
  )}</div>`;
  ...
  for (const toEmail of uniqueRecipients) {
    try {
      ...
      failed++;
      if (errors.length < 8) {
        errors.push(`${toEmail}: ${String((err as Error)?.message ?? err)}`);
      }
    }
  }
...
exports.platformGetMarketingDashboard = functions.https.onCall(async (data, context) => {
  ...
  const companyLightPreregistrationViews =
    eventTotals.get('company_light_preregistration_view') ?? 0;
  const companyLightPreregistrationSubmits =
    eventTotals.get('company_light_preregistration_submit') ?? 0;
  ...
  try {
    ...
    try {
      ...
      functions.logger.error(
        'notificarNovoCadastroAdministrativo office (publicCreateAccountantWorkspaceAccess)',
        notifyErr,
      );
    }
...
exports.platformGovernanceCompanyCancelAsaasBilling = functions.https.onCall(async (data, context) => {
  ...
  if (!companyId || isSupremePlatformCompany(companyId)) {
    throw new functions.https.HttpsError(
      'invalid-argument',
      'Informe uma empresa valida (nao aplicavel empresa suprema).',
    );
  }
  ...
  if (asTrimmedString(currentBilling.provider).toLowerCase() !== 'asaas' || !subscriptionId) {
    throw new functions.https.HttpsError(
      'failed-precondition',
      'A empresa nao possui assinatura Asaas para cancelamento administrativo.',
    );
  }
...
exports.platformGovernanceCompanyCancelPendingAsaasPayments = functions.https.onCall(async (data, context) => {
  ...
  if (asTrimmedString(billing.provider).toLowerCase() !== 'asaas' || !subscriptionId) {
    throw new functions.https.HttpsError(
      'failed-precondition',
      'Sem assinatura Asaas registada nesta empresa para localizar cobrancas mensais.',
    );
  }
  ...
  const paymentsRaw = paymentsResult?.data as unknown;
  ...
  let canceledCount = 0;
  let attempted = 0;
  const errorsOnCancel: Record<string, unknown>[] = [];

  for (const p of paymentsArray) {
    const pr = asRecord(p as Record<string, unknown>);
...
exports.platformGovernanceCompanySetSuspended = functions.https.onCall(async (data, context) => {
  ...
  if (typeof data?.suspend !== 'boolean') {
    throw new functions.https.HttpsError(
      'invalid-argument',
      'Informe o campo booleano suspend (true suspende pela governanca / false restaura valores guardados antes da suspensao).',
    );
  }
  ...
  const rawFlatCommercial: Record<string, unknown> = {...commercialSettings(settingsData)};
  ...
  if (suspend) {
    if (existingFreeze.active === true) {
      throw new functions.https.HttpsError(
        'failed-precondition',
        'Empresa já suspensa por esta ferramenta. Reative antes de repetir.',
      );
    }

    const priorLifecycleRaw = rawFlatCommercial.lifecycleStatus;
    const builtLifecycle =
      typeof priorLifecycleRaw === 'string' && priorLifecycleRaw.trim().length > 0
        ? priorLifecycleRaw
    ...
    const nextCommercialFlat: Record<string, unknown> = {
      ...rawFlatCommercial,
      allowLogin: false,
      lifecycleStatus: 'suspended',
  ...
  if (existingFreeze.active !== true) {
    throw new functions.https.HttpsError(
      'failed-precondition',
      'Esta empresa nao esta marcada como suspensa por esta ferramenta (freeze inativo).',
    );
  }

  const lifecycleRestore = asTrimmedString(existingFreeze.priorLifecycleStatus) || 'trial';
  let allowRestore =
    typeof existingFreeze.priorAllowLogin === 'boolean' ? !!existingFreeze.priorAllowLogin : true;
...
exports.assistantGetCompanyConfigStatus = functions.https.onCall(async (_data, context) => {
  ...
  const updatedAtIso =
    updatedAt instanceof admin.firestore.Timestamp ? updatedAt.toDate().toISOString() : '';
...
exports.assistantSaveCompanyApiKey = functions.https.onCall(async (data, context) => {
  ...
  if (!isSupremePlatformCompany(claims.companyId)) {
    throw new functions.https.HttpsError(
      'permission-denied',
      'Apenas o owner da empresa suprema pode cadastrar ou remover a credencial central do assistente.',
    );
  }
  ...
  if (apiKey.length === 0 && model.length > 0) {
    ...
    if (!existingKey) {
      throw new functions.https.HttpsError(
        'failed-precondition',
        'Cadastre a chave OpenAI antes de alterar apenas o modelo.',
      );
    }
    await secretRef.set(
      {
  ...
  if (apiKey.length < 20 || !apiKey.startsWith('sk-')) {
    throw new functions.https.HttpsError(
      'invalid-argument',
      'Informe uma chave valida da OpenAI (sk-...).',
    );
  }
...
function resolveCompanyDataFromSettingsOrUser(
  ...
  const settingsDemo =
    settingsCompanyData != null && isDemoPlaceholderCompanyData(settingsCompanyData);
  const userDemo =
    userCompanyData != null && isDemoPlaceholderCompanyData(userCompanyData);
